...
.NOTES
    ...
    Version : v4.7 (outcome reporting accuracy: when any step is
...
function Invoke-DiskCleanupRemediation {
    ...
    if ($cleanupSkipReason) {
        ...
        if ($r.TimedOut) {
            ...
            Add-Remediation -Step "DISM StartComponentCleanup" -Result "FAILED" -DurationSec $r.Duration -ErrorMessage ("{0} (see C:\Windows\Logs\DISM\dism.log)" -f (Format-ExitCode $r))
        }
    }
...
function Invoke-ComponentStoreRepair {
    ...
    if (-not (Test-RemainingBudget -NeededSec 60)) {
        ...
        if ($r.TimedOut) {
            ...
            Add-Remediation -Step "DISM /CheckHealth" -Result "FAILED" -DurationSec $r.Duration `
                -ErrorMessage "$exitTxt; treating as 'damage suspected' - escalating to ScanHealth (see C:\Windows\Logs\DISM\dism.log)"
        }
    }
    ...
    if (-not ($checkFlaggedDamage -or $ForceFullDISM)) {
        ...
        if ($r.TimedOut) {
            ...
            Add-Remediation -Step "DISM /ScanHealth" -Result "FAILED" -DurationSec $r.Duration -ErrorMessage ("{0} (see C:\Windows\Logs\DISM\dism.log)" -f (Format-ExitCode $r))
        }
    }
    ...
    if (-not $needRestore) {
        ...
        if ($r.TimedOut) {
            ...
            Add-Remediation -Step "DISM /RestoreHealth" -Result "FAILED" -DurationSec $r.Duration -ErrorMessage ("{0} (see C:\Windows\Logs\DISM\dism.log)" -f (Format-ExitCode $r))
        }
    }
