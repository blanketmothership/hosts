...
param(
    ...
    [switch]$AttemptRemediation = $true  # Set $false for audit-only / read-only mode
)
...
$Script:RemediationSkipReason = ""  # Populated by whichever gate blocks remediation
...
function Test-PendingReboot {
    ...
    try {
        ...
        if ($pfr -and $pfr.PendingFileRenameOperations -and
            ...
            $rebootResult.Sources.Add("PendingFileRenameOperations ($($pfr.PendingFileRenameOperations.Count) entries)")
            Write-Log ("  PENDING: PendingFileRenameOperations has {0} entries" -f
                $pfr.PendingFileRenameOperations.Count) -Level "WARN"
        }
    }
    ...
    $rebootResult.RebootPending = (
        ...
        $rebootResult.PFRPending     -or $rebootResult.SCCMPending -or
        $rebootResult.DomainJoinPending
    )
...
function Clear-WindowsUpdateCache {
    ...
    try {
        ...
        $cacheResult.Details = "Removed $totalRemoved cached items across $($cachePaths.Count) directories"
        Write-Log "  Cache cleared successfully. $($cacheResult.Details)"
    }
    catch {
        $cacheResult.Details = "Error during cache clear: $_"
        Write-Log $cacheResult.Details -Level "ERROR"
        Set-Status "WARN"
    }

    $Script:Results["CacheCleared"] = $cacheResult
    return $cacheResult
}
...
function Invoke-DISMRestoreHealth {
    ...
    try {
        ...
        $pinfo.Arguments              = "/Online /Cleanup-Image /RestoreHealth /NoRestart"
        ...
        $dismResult.ExitCode = $proc.ExitCode
        $dismResult.Output   = ($stdout + $stderr).Trim() -replace "`r`n", " | "
        $dismResult.Success  = ($proc.ExitCode -eq 0)

        if ($dismResult.Success) {
            Write-Log "  DISM RestoreHealth completed successfully (Exit 0)"
        } else {
            Write-Log ("  DISM RestoreHealth returned exit code {0}" -f $dismResult.ExitCode) -Level "WARN"
            Set-Status "WARN"
        }
    }
    catch {
        $dismResult.Output = "DISM execution failed: $_"
        Write-Log $dismResult.Output -Level "ERROR"
        Set-Status "WARN"
    }

    $Script:Results["DISM"] = $dismResult
    return $dismResult
}
...
function Invoke-WindowsUpdateDownload {
    ...
    $moduleAvailable = Get-Module -ListAvailable -Name PSWindowsUpdate -ErrorAction SilentlyContinue
    ...
    if ($moduleAvailable) {
        try {
            ...
            # Exclude SSUs again during download trigger
            $updates = Get-WindowsUpdate -MicrosoftUpdate -AcceptAll -IgnoreReboot `
                -NotCategory "Servicing Stack Updates" -ErrorAction Stop

            $wuResult.Method       = "PSWindowsUpdate"
            $wuResult.UpdatesFound = $updates.Count

            if ($updates.Count -gt 0) {
                Write-Log "  $($updates.Count) update(s) queued for download/install."
                $updates | ForEach-Object { Write-Log "    -> $($_.Title)" }
                $wuResult.UpdatesQueued = $updates.Count
                $wuResult.Success       = $true
            } else {
                Write-Log "  No new updates available."
                $wuResult.Success = $true
                $wuResult.Details = "System is fully patched."
            }
        }
        catch {
            Write-Log "  PSWindowsUpdate error: $_" -Level "WARN"
            $wuResult.Details = "PSWindowsUpdate error: $_"
        }
    }

    # Fallback: WUA COM object (built-in, no module needed)
    if (-not $wuResult.Success -or $wuResult.Method -eq "Unknown") {
        try {
            Write-Log "  Using WUA COM object (wuapilib)..."
            $wuSession     = New-Object -ComObject Microsoft.Update.Session
            $wuSearcher    = $wuSession.CreateUpdateSearcher()
            $searchResult  = $wuSearcher.Search("IsInstalled=0 AND Type='Software' AND IsHidden=0")

            # Filter out SSUs
            $filteredUpdates = @($searchResult.Updates | Where-Object {
                $title = $_.Title
                $isSSU = ($title -like "*Servicing Stack*") -or ($title -like "*Service Stack*")
                -not $isSSU
            })

            $wuResult.Method       = "WUA COM"
            $wuResult.UpdatesFound = $filteredUpdates.Count

            if ($filteredUpdates.Count -gt 0) {
                Write-Log "  $($filteredUpdates.Count) update(s) found. Queueing download..."
                ...
                $wuResult.UpdatesQueued = $filteredUpdates.Count
                $wuResult.Details       = "Download result code: $($dlResult.ResultCode)"
                $wuResult.Success       = ($dlResult.ResultCode -in 2, 3)  # 2=Succeeded, 3=SucceededWithErrors

                Write-Log "  Download result code: $($dlResult.ResultCode)"
            } else {
                Write-Log "  No new updates available (SSUs excluded)."
                $wuResult.Success = $true
                $wuResult.Details = "System is fully patched (SSUs excluded)."
            }
        }
        catch {
            $wuResult.Details = "WUA COM error: $_"
            Write-Log $wuResult.Details -Level "ERROR"
            Set-Status "WARN"
        }
    }

    $Script:Results["WindowsUpdate"] = $wuResult
    return $wuResult
}
...
function Write-ComplianceReport {
    ...
    if ($disk) {
        $diskFlag = switch ($disk["Status"]) {
            "FAIL"  { "!! INSUFFICIENT - REMEDIATION BLOCKED !!" }
    ...
    if ($reboot) {
        ...
        Row "Reboot Pending"         $(if ($reboot["RebootPending"]) {"YES"} else {"No"})  $rebootFlag
        Row "CBS Pending"            $(if ($reboot["CBSPending"])      {"Yes"} else {"No"})
        Row "Windows Update Pending" $(if ($reboot["WUPending"])       {"Yes"} else {"No"})
        Row "File Rename Pending"    $(if ($reboot["PFRPending"])      {"Yes"} else {"No"})
    ...
    # ---- Remediation: Cache Clear ----
    $cache = $AllResults["CacheCleared"]
    Header "REMEDIATION - WINDOWS UPDATE CACHE CLEAR"
    if ($cache) {
        Row "Success"    $(if ($cache["Success"]) {"Yes"} else {"No - see execution log"}) $(if (-not $cache["Success"]) {"WARNING"})
        Row "Details"    $cache["Details"]
    } else {
        $skipMsg = if ($Script:RemediationSkipReason) { $Script:RemediationSkipReason } else { "Did not run" }
        Write-Output "  Skipped  - $skipMsg"
    }

    # ---- Remediation: DISM ----
    $dism = $AllResults["DISM"]
    Header "REMEDIATION - DISM RESTOREHEALTH"
    if ($dism) {
        Row "Success"    $(if ($dism["Success"]) {"Yes"} else {"No - see execution log"}) $(if (-not $dism["Success"]) {"WARNING"})
        Row "Exit Code"  $dism["ExitCode"]
    } else {
        $skipMsg = if ($Script:RemediationSkipReason) { $Script:RemediationSkipReason } else { "Did not run" }
        Write-Output "  Skipped  - $skipMsg"
    }
    ...
    $wu = $AllResults["WindowsUpdate"]
    Header "REMEDIATION - WINDOWS UPDATE DOWNLOAD"
    if ($wu) {
        Row "Detection Method"   $wu["Method"]
        Row "Updates Found"      $wu["UpdatesFound"]
        Row "Updates Queued"     $wu["UpdatesQueued"]
        Row "Details"            $wu["Details"]
        Row "Success"            $(if ($wu["Success"]) {"Yes"} else {"No - see execution log"}) $(if (-not $wu["Success"]) {"WARNING"})
    } else {
        $skipMsg = if ($Script:RemediationSkipReason) { $Script:RemediationSkipReason } else { "Did not run" }
        Write-Output "  Skipped  - $skipMsg"
    }
...
# ============================================================
Write-Log "############################################################"
Write-Log " Invoke-PatchComplianceCheck - Starting  [v9]"
...
try {
    ...
    $servicesOkForRemediation = $wuSvcInfo["AllMandatoryOK"]
    $rebootOkForRemediation   = -not $rebootInfo["RebootPending"]
    $timeSyncOkForRemediation = ($timeSyncInfo["Status"] -in @("PASS", "WARN", "UNKNOWN"))  # FAIL blocks

    if (-not $eolInfo["IsEOL"] -and $AttemptRemediation) {
        if (-not $diskOkForRemediation) {
            $Script:RemediationSkipReason = "Insufficient disk space: $($diskInfo['FreeGB']) GB free, $($diskInfo['MinRequiredGB']) GB required"
            ...
            $Script:RemediationSkipReason = "One or more mandatory Windows Update services are not healthy - resolve before re-running"
            ...
            $Script:RemediationSkipReason = "Reboot is pending ($($rebootInfo['Sources'] -join '; ')) - reboot the system first"
            ...
            $tsOffset = $timeSyncInfo["OffsetFormatted"]
            $Script:RemediationSkipReason = "Clock offset ($tsOffset) exceeds Kerberos limit - run w32tm /resync /force first"
            Write-Log ("=== Remediation SKIPPED - Clock offset exceeds Kerberos limit ({0}). Run w32tm /resync /force first. ===" -f $tsOffset) -Level "ERROR"
            Set-Status "FAIL"
        } else {
        ...
        $Script:RemediationSkipReason = "OS is End of Life with no valid ESU - manual upgrade required"
        Write-Log "=== Remediation SKIPPED - OS is End of Life with no valid ESU. Manual upgrade required. ===" -Level "WARN"
        Set-Status "FAIL"
    } else {
        $Script:RemediationSkipReason = "AttemptRemediation is set to false (audit-only mode)"
