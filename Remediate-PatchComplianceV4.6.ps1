...
.NOTES
    ...
    Version : v4.6 (re-applied V4.1 .Count fix to report tally that
...
function Invoke-ComponentStoreRepair {
    ...
    if (-not (Test-RemainingBudget -NeededSec 60)) {
        ...
        if ($r.TimedOut) {
            ...
            $checkFlaggedDamage = $true
            $exitTxt = if ($null -ne $r.ExitCode) { "exit $($r.ExitCode)" } else { "no exit code returned" }
            Add-Remediation -Step "DISM /CheckHealth" -Result "FAILED" -DurationSec $r.Duration `
                -ErrorMessage "$exitTxt; treating as 'damage suspected' - escalating to ScanHealth"
        }
    }
...
# ============================================================
Write-Log "############################################################"
Write-Log " Remediate-PatchCompliance - Starting  [v4.6]"
