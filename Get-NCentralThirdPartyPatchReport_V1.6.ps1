...
.NOTES
    Get-NCentralThirdPartyPatchReport_V1.6.ps1
...
try {
    ...
    $cacheUsage     = $null    # last-seen FileCacheServiceAgent cache size (MB)
    ...
    foreach ($lf in $logFiles) {
        ...
        foreach ($line in $lines) {
            ...
            # ERROR and cache-usage lines matter - cheap ordinal checks first
            if ($isCacheFile -and $line.IndexOf('ERROR') -lt 0 -and $line.IndexOf('CurrentCache[') -lt 0) { continue }
            ...
            # ---- cache usage tracking (newest wins) --------------------------
    ...
    if ($cacheUsage) {
        # best-effort read of the configured cache limit
        $cacheLimitMB = $null
        ...
        $limStr = if ($cacheLimitMB) { ' (configured limit ~{0} MB)' -f $cacheLimitMB } else { '' }
        Write-Output ('   Patch cache usage       : {0} MB last seen {1:yyyy-MM-dd HH:mm}{2}' -f $cacheUsage.MB, $cacheUsage.Time, $limStr)
        $nearCap = ($cacheLimitMB -and $cacheUsage.MB -ge (0.9 * $cacheLimitMB)) -or
                   ((-not $cacheLimitMB) -and $cacheUsage.MB -ge 1800)
        if ($nearCap) {
            Write-Output '   WARNING: patch cache is at/near its size limit - the classic cause'
            Write-Output '   of caching timeouts. Remediation clears it; to stop it refilling,'
