...
.NOTES
    ...
    Version : v4.22 (Fix release: ESU/EOL lifecycle tables updated
        ...
        if ($auPol) {
            if (& $hasProp $auPol 'NoAutoUpdate') {
                ...
                if ($r.NoAutoUpdate -eq 1) {
                    $r.BlockingReasons += "NoAutoUpdate=1 - automatic updates disabled entirely (blocks feature AND quality updates; deliberate config - reported, never auto-removed)"
                }
            }
...
function Write-ComplianceReport {
    ...
    if ($upause) {
        ...
        foreach ($pd in @($upause["Details"])) { Row "  $pd" "" }
    ...
    if ($fur -and $fur["Applicable"]) {
        ...
        Row "AU: NoAutoUpdate / AUOptions"  ("{0} / {1}" -f $naDisp, $auDisp)  $(if ($fur["NoAutoUpdate"] -eq 1) {"!! AUTOMATIC UPDATES DISABLED !!"})
        ...
        if ($null -ne $fur["AppraiserBlocked"]) {
            ...
            foreach ($af in @($fur["AppraiserFindings"])) { Row "  $af" "" }
        } else {
