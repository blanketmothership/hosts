...
# ============================================================
param(
    [int]   $PatchStaleDays     = 30,    # Days before a missing LCU is considered overdue
...
function Get-LastCumulativeUpdate {
    ...
    # Fallback: query WMI Win32_QuickFixEngineering for KB descriptions (Get-HotFix 
    # sometimes lacks descriptions). Cross-reference with WU history if possible.
    if ($lcuHistory.Count -eq 0) {
        Write-Log "  Get-HotFix returned no LCUs; attempting WMI/WU history query..." -Level "WARN"
...
function Test-WindowsUpdateServices {
    ...
    foreach ($svcDef in $serviceDefinitions) {
        ...
        try {
            ...
            # A service is healthy if it is Running, OR if it is stopped but
            # configured for Manual/Trigger-Start (acceptable for trustedinstaller,
            # msiserver, dosvc which only run on demand).
            $isRunning     = ($svc.Status -eq 'Running')
            $isOnDemandOk  = ($svc.Status -eq 'Stopped') -and
                             ($svc.StartType -in @('Manual', 'Automatic'))

            if ($isRunning) {
                $row.Healthy = $true
                $row.Note    = "Running"
                Write-Log ("  OK       {0,-20} : {1} ({2})" -f $svcName, $svc.Status, $svc.StartType)
            } elseif ($isOnDemandOk -and -not $mandatory) {
                # Advisory-only services that are stopped on-demand are fine
                $row.Healthy = $true
                $row.Note    = "Stopped (on-demand) - acceptable"
...
function Write-ComplianceReport {
    ...
    if ($cache) {
        ...
        Write-Output "  Skipped  (OS is EOL with no ESU, or audit-only mode enabled)"
    }
...
try {
    ...
    $osBuild = [string]$sysInfo["OS_BuildNumber"]
    $eolInfo = Test-WindowsEOL -BuildNumber $osBuild
