...
.DESCRIPTION
    ...
    PHASE 2 - REMEDIATE  (V4: gated; runs only what Phase 1 indicates)
        | Step                          | Runs when                                     |
        | ----------------------------- | --------------------------------------------- |
        | Disk cleanup                  | Free < (MinFreeGB+5) OR -ForceCleanup         |
        ...
        | Clear update pauses           | active pause AND (-ClearUpdatePauses OR       |
        |                               |   -UnblockFeatureUpdates)                     |

        v4.22: a serious pending reboot (CBS/WU/SCCM/domain-join)
        gates Component Reset, ScanHealth/RestoreHealth, SFC,
        StartComponentCleanup and update installs; the run reports
        REBOOT_REQUIRED instead. PFR-only pending never gates.

        v4.25: -InstallApplicable never installs the "Upgrades"
        category (feature updates / OS version changes) unless
        -AllowFeatureUpdate is also specified.
...
.NOTES
    ...
    Version : v4.25 (Dead-code + safety release: unused pre-v4.3
                     Invoke-CapturedCommand helper and unused
                     AnyRebootPending flag removed; .bak retention
                     prune now runs on every Phase 2 pass instead of
                     only after a component reset (healthy hosts
                     never pruned old backups); -InstallApplicable
                     now excludes the "Upgrades" category so
                     AcceptAll can never stage a Windows 11 /
                     feature update as a side effect - new
                     -AllowFeatureUpdate switch opts back in;
                     0x800f0915 added to the DISM triage table;
                     transient WUA-search failures annotated.)
              v4.24 (Field fixes from the V4.23 live run: bookkeeping
                     probes now count RAW Get-HotFix/QFE entries. QFE
                     Description never contains "Cumulative" on modern
                     Windows, so the old LCU-pattern count was always
                     0, which marked every healthy host DEGRADED and
                     triggered a destructive SoftwareDistribution
                     reset per 24h - the script was wiping the very
                     history it was diagnosing. Reset trigger narrowed
                     to the true corruption signature (WUA sees
                     history, both legacy lookups return zero KBs);
                     LCU title match broadened (bare "Cumulative
                     Update"; .NET excluded); recent history titles
                     are logged when nothing matches; QFE-based
                     approximate LCU-date fallback when WUA history
                     is empty; Stop/Start-Service warning spam
                     suppressed.)
              v4.23 (Field fixes from the first V4.22 live run:
                     Phase 3 re-check output captured (raw Name/Value
                     dump no longer leaks into the report); final
                     status capped at WARN when only demoted/deferred
                     items re-applied FAIL during re-check (exit 1
                     not 2, unless the EOL floor is FAIL);
                     NoAutoUpdate=1 demoted to informational -
                     expected managed state under N-central PME;
                     AUOptions=1 meaning added; report cosmetics
                     (appraiser/pause/safeguard detail rows,
                     before/after column width, footer casing).)
              v4.22 (Fix release: ESU/EOL lifecycle tables updated
                     (Win10 22H2 ESU, LTSC, Enterprise/Education
                     dates, 25H2, Server 2025 build); feature-update
                     policy probe parse fix; DISM /Quiet dropped from
                     health checks so verdict matching works;
                     wuauserv Manual/Stopped treated as healthy; 24h
                     recent-reset guard applies to all reset
                     triggers; serious pending reboot gates
                     destructive steps; pending.xml rename requires
                     a reboot since file write; Phase 3 recheck +
                     exit codes no longer mask residual WARN/FAIL;
                     WSUS-managed hosts not scanned via
                     -MicrosoftUpdate; 32-bit host relaunch guard;
                     Fast Startup check; NTP flag-suffix fix;
                     version-pin classifier (BLOCKING / TARGETING /
                     INACTIVE / STALE / UNRECOGNISED) with gated
                     unblock; update-pause, legacy OS-upgrade-block,
                     NoAutoUpdate and hardware-appraiser detection;
                     readiness check extended to 24H2 hosts;
                     always-on update-pause check on every SKU
                     (quality pause = monthly patching suspended)
                     with opt-in -ClearUpdatePauses remediation.)
              v4.21 (Internal wall-clock time budget removed. The
                     script no longer self-terminates at a fixed
                     minute cap; steps (including DISM RestoreHealth)
                     run to completion, bounded only by N-central's
                     own task execution timeout. -MaxRuntimeMinutes
                     defaults to 0 = unlimited; pass a positive value
                     to re-impose an internal cap. Budget gates and
                     timeout wrappers are retained and treat unlimited
                     as effectively infinite remaining budget.)
              v4.20 (Changelog tightened: per-version entries
                     condensed to single-line summaries and
                     cross-references between version entries
                     removed. No code or behaviour changes.)
              v4.19 (sfc /scannow fast non-zero exit treated as
                     FAILED; wuapi.dll added to deprecated-DLLs
                     skip list.)
              v4.18 (Captured-process wrapper rewritten to use
                     [System.Diagnostics.Process]::Start with async
                     stream reads; dism.log tail fallback when no
                     captured output is produced; SFC progress-line
                     filter extended.)
              v4.17 (Test-FeatureUpdateReadiness Phase 1 check +
                     gated Phase 2 remediation; -UnblockFeatureUpdates
                     switch for policy unblock.)
              v4.16 (Budget-aware step-launch logs;
                     exit-code-null diagnostics added to the
                     captured-command wrapper.)
              v4.15 (Sanitised: removed author, repo, and
                     per-incident host references.)
              v4.14 (Test-DiskSpace format-string fix in WARN/FAIL
                     branches.)
              v4.13 (Wall-clock budget default raised to 120 min;
                     per-step DISM/SFC TimeoutSec caps removed;
                     both timeout wrappers default to remaining
                     budget when TimeoutSec is omitted.)
              v4.12 (Phase 1 LCU lookup reordered to make WUA COM
                     the primary source; Get-HotFix and
                     Win32_QuickFixEngineering probes wrapped in
                     a 60s job-based timeout; pre-flight booleans
                     default to Unknown.)
              v4.11 (DISM /Quiet on every call; wrapper redirects
                     stderr to a temp file to close a
                     stderr-buffer-fill hang.)
              v4.10 (DISM LOG ERROR SUMMARY always renders on
                     failure; sfc null/empty exit treated as FAILED;
                     new REBOOT_REQUIRED outcome for CBS-only
                     pending-reboot blockers.)
              v4.9  (DISM log auto-triage with HRESULT extraction
                     and translation; DISM LOG ERROR SUMMARY
                     report section.)
              v4.8  (Symmetric ScanHealth escalation to
                     RestoreHealth on non-zero exit.)
              v4.7  (Truncated-step demote REMEDIATED -> PARTIAL;
                     dynamic version footer.)
              v4.6  (.Count fix in report tally; CheckHealth
                     failure escalates to ScanHealth; WaitForExit
                     guarantees ExitCode populated.)
              v4.5  (Tightened /StartComponentCleanup gating;
                     disk-low buffer raised to MinFreeGB+5.)
              v4.3  (Hard time budget; DISM/SFC bounded with
                     kill-on-timeout; report guaranteed via
                     try/finally.)
              v4.2  (Report headline leads with post-remediation
                     outcome.)
              v4.1  (StrictMode fix in report tally.)
              v4    (Gating layer; deprecated-DLL handling; .bak
                     prune; -InstallApplicable switch.)
...
param(
    ...
    # v4.25: -InstallApplicable excludes the "Upgrades" category (feature
    # updates / OS version changes) by default, so AcceptAll can never
    # stage a Windows 11 upgrade as a side effect. Pass this switch
    # together with -InstallApplicable to deliberately allow it.
    ...
    # v4.22: opt-in clearing of ACTIVE Windows Update pauses (WUfB policy
    # values or Settings-app pause) on any SKU. Pauses can be deliberate
    # (e.g. dodging a bad KB), so the always-on check only WARNs by
    # default; pass this switch to clear them. -UnblockFeatureUpdates
    # also implies pause clearing.
    [switch] $ClearUpdatePauses,

    # Internal wall-clock budget for the entire script.
    # v4.21: default is now 0 = UNLIMITED. The script no longer imposes
    # its own minute cap; long steps (notably DISM /RestoreHealth) run to
    # completion and the only ceiling is N-central's own task execution
    # timeout. Set the N-central "Run a Script" / Scheduled Task timeout
    # to whatever wall you want (e.g. 3 hours) - that is now the effective
    # limit. Pass a positive -MaxRuntimeMinutes (e.g. 110) only if you
    # deliberately want the script to self-terminate before the host wall
    # so it can guarantee the final report ships; with 0 (unlimited) the
    # on-host transcript remains the fallback record if N-central kills
    # the process at its own wall.
...
# ============================================================
#  32-BIT HOST GUARD (v4.22)
#  N-central can launch scripts in 32-bit PowerShell. Under WOW64,
#  DISM/SFC misbehave and regsvr32/System32 paths silently redirect
#  to SysWOW64. Relaunch in the native 64-bit host via sysnative,
#  forwarding all bound parameters and the child's exit code.
# ============================================================
if ($env:PROCESSOR_ARCHITEW6432) {
    $nativePS = Join-Path $env:WINDIR 'sysnative\WindowsPowerShell\v1.0\powershell.exe'
    if (Test-Path $nativePS) {
        Write-Host "[INFO] 32-bit PowerShell detected on 64-bit Windows; relaunching in 64-bit host..."
        $fwdArgs = @('-NoProfile','-ExecutionPolicy','Bypass','-File',"`"$PSCommandPath`"")
        foreach ($kv in $PSBoundParameters.GetEnumerator()) {
            if ($kv.Value -is [switch]) {
                if ($kv.Value.IsPresent) { $fwdArgs += "-$($kv.Key)" }
            } else {
                $fwdArgs += "-$($kv.Key)"
                $fwdArgs += "$($kv.Value)"
            }
        }
        $nativeProc = Start-Process -FilePath $nativePS -ArgumentList $fwdArgs -NoNewWindow -Wait -PassThru
        exit $nativeProc.ExitCode
    }
    Write-Host "[WARN] 32-bit PowerShell on 64-bit Windows and sysnative is unavailable; DISM/SFC/regsvr32 results may be unreliable."
}
...
#  GLOBAL STATE
# ============================================================
$Script:Version        = "v4.25"
$Script:ScriptName     = "Remediate-PatchCompliance$($Script:Version.ToUpper()).ps1"
...
# v4.21: MaxRuntimeMinutes <= 0 means UNLIMITED - no internal deadline.
# All budget gates and timeout wrappers keep functioning; they simply
# see an effectively infinite remaining budget, so nothing is skipped
# or cut short by an internal cap. N-central's own task timeout is the
# only ceiling. A positive MaxRuntimeMinutes restores a hard deadline.
$Script:UnlimitedBudget = ($MaxRuntimeMinutes -le 0)
$Script:DeadlineUtc = if ($Script:UnlimitedBudget) {
    [DateTime]::MaxValue
} else {
    $Script:StartUtc.AddMinutes($MaxRuntimeMinutes)
}
...
# v4.21: when the budget is unlimited we report a large finite ceiling
# (30 days) rather than [int] math on DateTime.MaxValue, which would
# overflow. 30 days far exceeds any N-central task wall, so this value
# never binds before N-central's own timeout - i.e. effectively infinite
# while keeping every downstream timeout/deadline calculation valid.
$Script:UnlimitedBudgetSec = 30 * 24 * 3600   # 2,592,000

function Get-RemainingBudgetSec {
    if ($Script:UnlimitedBudget) { return $Script:UnlimitedBudgetSec }
...
function Format-Budget {
    if ($Script:UnlimitedBudget) { return "unlimited (no internal cap; N-central task timeout is the only ceiling)" }
...
#  Runs a native command with captured output and kills the
#  process if it exceeds TimeoutSec. Returns a hashtable that includes a
...
#    Windows 10 22H2             ESU Y1 ends 2026-10-13
#                                ESU Y2 ends 2027-10-12
#                                ESU Y3 ends 2028-10-10
# ============================================================
function Get-ESULicenseStatus {
    ...
    $esuTable = @{
        ...
        # Windows 10 22H2 (Build 19045) - EOS 2025-10-14. Consumer and
        # commercial ESU programs available (v4.22).
        "19045" = @{
            Label   = "Windows 10 22H2"
            Capable = $true
            Years   = @{
                "Y1" = [datetime]"2026-10-13"
                "Y2" = [datetime]"2027-10-12"
                "Y3" = [datetime]"2028-10-10"
            }
        }
    }
...
function Test-WindowsEOL {
    ...
    $lifecycleTable = @{
        ...
        "26200" = @("Windows 11 25H2 (Home/Pro)",     [datetime]"2027-10-12")
        # Windows Server 2025 shares build 26100 with Windows 11 24H2 and is
        # disambiguated by the caption override below (v4.22). The old
        # "26080" entry was an Insider build and never matched RTM hosts.
    }
    ...
    # v4.22: several builds are shared across products/editions with
    # different lifecycle dates (LTSC, Enterprise/Education 36-month
    # servicing, Server 2025 on build 26100). Disambiguate by caption
    # before falling back to the generic (Home/Pro) table.
    $osCaption = ""
    try { $osCaption = [string](Get-CimInstance -ClassName Win32_OperatingSystem -ErrorAction Stop).Caption } catch {}

    $entry = $null
    if ($osCaption -match 'LTSC') {
        if ($BuildNumber -eq '19044') {
            $entry = if ($osCaption -match 'IoT') { @("Windows 10 IoT Enterprise LTSC 2021", [datetime]"2032-01-13") }
                     else                          { @("Windows 10 Enterprise LTSC 2021",     [datetime]"2027-01-12") }
        } elseif ($BuildNumber -eq '17763' -and $osCaption -notmatch 'Server') {
            $entry = @("Windows 10 Enterprise LTSC 2019", [datetime]"2029-01-09")
        }
    } elseif ($BuildNumber -eq '26100' -and $osCaption -match 'Server 2025') {
        $entry = @("Windows Server 2025", [datetime]"2034-10-10")
    } elseif ($osCaption -match 'Enterprise|Education') {
        switch ($BuildNumber) {
            '22621' { $entry = @("Windows 11 22H2 (Enterprise/Education)", [datetime]"2025-10-14") }
            '22631' { $entry = @("Windows 11 23H2 (Enterprise/Education)", [datetime]"2026-11-10") }
            '26100' { $entry = @("Windows 11 24H2 (Enterprise/Education)", [datetime]"2027-10-12") }
            '26200' { $entry = @("Windows 11 25H2 (Enterprise/Education)", [datetime]"2028-10-10") }
        }
    }
    if (-not $entry -and $lifecycleTable.ContainsKey($BuildNumber)) {
...
function Get-LastCumulativeUpdate {
    ...
    # Exclusion keywords - titles matching these are never counted as
    # OS LCUs (SSUs, and v4.24: .NET Framework cumulative updates).
    $ssuPatterns = @(
        "Servicing Stack",
        "Servicing stack",
        "SSU",
    ...
    # v4.24: bare "Cumulative Update" added - title conventions have
    # drifted across Win11 24H2/25H2; the .NET exclusion above keeps
    # framework CUs out of the OS-LCU count.
    $lcuPatterns = @(
    ...
    try {
        ...
        if ($totalHistory -gt 0) {
            ...
            if ($lcuHistory.Count -gt 0) {
                ...
                # v4.24: surface the most recent successful installs so the
                # operator can see WHY nothing matched (title drift, or a
                # history that only contains Defender/driver/Store entries).
                try {
                    $recentInstalls = @($wuaHistory | Where-Object { $_.ResultCode -eq 2 -and $_.Operation -eq 1 } |
                                        Sort-Object Date -Descending | Select-Object -First 5)
                    foreach ($h in $recentInstalls) {
                        Write-Log ("    recent install: [{0}] {1}" -f ([datetime]$h.Date).ToString('yyyy-MM-dd'), $h.Title)
                    }
                } catch {}
            }
        } else {
    ...
    if ($hotfixProbe.TimedOut) {
        ...
        # v4.24: count RAW entries. QFE Description on modern Windows is only
        # ever "Security Update"/"Update"/"Hotfix" - it never contains
        # "Cumulative", so the old LCU-pattern count was permanently 0 and
        # falsely marked every healthy host DEGRADED (triggering a
        # destructive component reset per 24h).
        $hotfixCount = $hotfixHistory.Count

        # Secondary fallback: if WUA COM came back empty, approximate the
        # LCU date from the newest Security Update/Update entry.
        if ($lcuHistory.Count -eq 0 -and $hotfixCount -gt 0) {
            ...
            if ($hotfixLcus.Count -gt 0) {
                Write-Log ("  WUA COM empty; approximating LCU date from Get-HotFix ({0} Security Update/Update entries)" -f $hotfixLcus.Count)
                $lcuHistory = $hotfixLcus
                $dataSource = "Get-HotFix (approximate - newest Security Update/Update entry)"
            }
        }
    }
    ...
    if ($wmiProbe.TimedOut) {
        ...
        # v4.24: raw count (see Get-HotFix probe note above).
        $wmiCount = $wmiHistory.Count

        # Tertiary fallback: if both WUA COM and Get-HotFix came back
        # empty, approximate from the newest Security Update/Update entry.
        if ($lcuHistory.Count -eq 0 -and $wmiCount -gt 0) {
            $wmiLcus = @($wmiHistory | Where-Object {
                "$($_.Description)" -match '^(Security Update|Update)$'
            } | Sort-Object InstalledOn -Descending)
            if ($wmiLcus.Count -gt 0) {
                Write-Log ("  WUA COM and Get-HotFix empty; approximating from Win32_QuickFixEngineering ({0} entries)" -f $wmiLcus.Count)
                $lcuHistory = $wmiLcus
                $dataSource = "Win32_QuickFixEngineering (approximate - newest Security Update/Update entry)"
            }
        }
    }
    ...
    $lcuResult = [ordered]@{
        ...
        WMICount            = $wmiCount
        # v4.24: Healthy = at least one legacy lookup returns ANY KBs at
        # all (raw counts). Degraded = both return zero while WUA COM has
        # history - the true DataStore-corruption signature. Probe
        # timeouts are -1 (Unknown) and never trigger a reset.
...
function Test-WindowsUpdateServices {
    ...
    foreach ($svcDef in $serviceDefinitions) {
        ...
        try {
            ...
            # v4.22: wuauserv added - on Win10/11 it is Manual (Trigger
            # Start) and legitimately Stopped at idle. WUA COM
            # reachability (tested below) is the real WU-engine signal.
            $demandStartServices = @('wuauserv', 'bits', 'trustedinstaller', 'msiserver', 'dosvc')
...
function Test-PendingReboot {
    ...
    try {
        ...
        if ($pfr -and $pfr.PendingFileRenameOperations -and
            ...
            # indicate a problem on their own; no WARN is raised and (v4.22)
            # PFR does NOT gate remediation - only serious sources do.
    ...
    # (v4.25: unused AnyRebootPending convenience flag removed - the
    # Phase 2 gate keys off RebootPending; PFR never gates.)
    ...
    if ($rebootResult.RebootPending) {
        ...
        $rebootResult.RecommendedAction =
            "PendingFileRenameOperations detected. Informational only - entries clear at next reboot."
        Write-Log "  INFO: Only PendingFileRenameOperations detected - no WARN raised and remediation is NOT gated (PFR is normal)."
    } else {
...
# ============================================================
#  SECTION 3e2 - FAST STARTUP CHECK (v4.22)
#  Windows "Fast Startup" (hiberboot) makes Shut Down hibernate the
#  kernel session instead of performing a real boot. Users who only
#  ever shut down never complete a true reboot, so pending updates
#  never finalise and uptime accumulates. Detection-only: escalates
#  to WARN only when there is evidence of interference (uptime past
#  the warning threshold or a serious pending reboot); otherwise
#  reported as INFO, since Fast Startup is ON by default on most
#  client installs.
# ============================================================
function Test-FastStartup {
    Write-Log "=== Checking Fast Startup (hiberboot) ==="
    ...
    try {
        ...
        if ($os.ProductType -ne 1) {
            $fsResult.Note = "Server SKU - Fast Startup not applicable."
            Write-Log "  Skipping - not a client OS."
            $Script:Results["FastStartup"] = $fsResult
            return $fsResult
        }
        $fsResult.Applicable = $true

        $fsKey  = "HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Power"
        $fsItem = Get-ItemProperty -Path $fsKey -Name HiberbootEnabled -ErrorAction SilentlyContinue

        if ($null -eq $fsItem) {
            $fsResult.Enabled = $false
            $fsResult.Status  = "PASS"
            $fsResult.Note    = "HiberbootEnabled not set - Fast Startup off or hibernation unavailable."
            Write-Log "  PASS: HiberbootEnabled not present."
        } elseif ([int]$fsItem.HiberbootEnabled -eq 1) {
            $fsResult.Enabled = $true
            $upNow  = $Script:Results["Uptime"]
            $rebNow = $Script:Results["PendingReboot"]
            $interfering = ($upNow -and $upNow["UptimeDays"] -gt $UptimeWarnDays) -or
                           ($rebNow -and $rebNow["RebootPending"])
            if ($interfering) {
                $fsResult.Status = "WARN"
                $fsResult.Note   = "Fast Startup is ON and uptime/pending-reboot evidence suggests real reboots are not happening. 'Shut down' does not finalise patches - use Restart, or disable via HiberbootEnabled=0 / powercfg /hibernate off."
                Write-Log "  WARN: Fast Startup (HiberbootEnabled=1) is ON and appears to be preventing real reboots." -Level "WARN"
                Set-Status "WARN"
            } else {
                $fsResult.Status = "INFO"
                $fsResult.Note   = "Fast Startup is ON (Windows default). No interference detected yet; note that 'Shut down' does not perform a real reboot."
                Write-Log "  INFO: Fast Startup is ON (default); no interference evidence at this time."
            }
        } else {
            $fsResult.Enabled = $false
            $fsResult.Status  = "PASS"
            $fsResult.Note    = "Fast Startup disabled (HiberbootEnabled=0)."
            Write-Log "  PASS: Fast Startup disabled (HiberbootEnabled=0)."
        }
    } catch {
        $fsResult.Status = "ERROR"
        $fsResult.Note   = "Fast Startup check failed: $_"
        Write-Log "  Fast Startup check failed: $_" -Level "WARN"
    }
...
# ============================================================
#  SECTION 3e3 - UPDATE PAUSE STATE (v4.22)
#  Shared scan + always-on Phase 1 check. Windows lets updates be
#  paused via WUfB policy values or the Settings app (UX\Settings).
#  An active QUALITY pause suspends monthly security patching -
#  exactly the condition this script exists to catch - so this
#  check runs and WARNs on every SKU, server or client. The
#  feature-update readiness check consumes the same scan for its
#  client-side feature-offer view.
# ============================================================
function Get-UpdatePauseScan {
    ...
    $nowDt = Get-Date
    $pauseSources = @(
        @{ Key = "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate"; Origin = "policy" },
        @{ Key = "HKLM:\SOFTWARE\Microsoft\WindowsUpdate\UX\Settings";      Origin = "Settings-app" }
    )
    foreach ($psrc in $pauseSources) {
        try {
            $pk = Get-ItemProperty -Path $psrc.Key -ErrorAction SilentlyContinue
            if (-not $pk) { continue }

            # Master expiry (Settings-app pause) applies to both types
            if ((& $hasProp $pk 'PauseUpdatesExpiryTime') -and $pk.PauseUpdatesExpiryTime) {
                $expDt = [datetime]::MinValue
                $expOk = [datetime]::TryParse("$($pk.PauseUpdatesExpiryTime)", [ref]$expDt)
                if ($expOk -and $expDt -gt $nowDt) {
                    $scan.FeatureActive = $true
                    $scan.QualityActive = $true
                    $reason = "{0}: ALL updates paused until {1} (PauseUpdatesExpiryTime)" -f $psrc.Origin, $expDt.ToString('yyyy-MM-dd')
                    $scan.FeatureReasons += $reason
                    $scan.QualityReasons += $reason
                    $scan.Details        += $reason
                } elseif ($expOk) {
                    $scan.Details += ("{0}: PauseUpdatesExpiryTime expired {1}" -f $psrc.Origin, $expDt.ToString('yyyy-MM-dd'))
                }
            }

            foreach ($kind in 'Feature','Quality') {
                $endName   = "Pause${kind}UpdatesEndTime"
                $startName = "Pause${kind}UpdatesStartTime"
                $endVal = $null; $startVal = $null
                if ((& $hasProp $pk $endName)   -and $pk.$endName)   { $endVal   = "$($pk.$endName)" }
                if ((& $hasProp $pk $startName) -and $pk.$startName) { $startVal = "$($pk.$startName)" }
                if (-not $endVal -and -not $startVal) { continue }

                $active = $false; $desc = ""
                $endDt = [datetime]::MinValue; $endOk = $false
                if ($endVal) { $endOk = [datetime]::TryParse($endVal, [ref]$endDt) }
                if ($endOk) {
                    $active = ($endDt -gt $nowDt)
                    $desc   = "until $($endDt.ToString('yyyy-MM-dd'))"
                } elseif ($startVal) {
                    $startDt = [datetime]::MinValue
                    $startOk = [datetime]::TryParse($startVal, [ref]$startDt)
                    if ($startOk) {
                        $active = ($startDt.AddDays(35) -gt $nowDt)   # WU honours max 35 days from start
                        $desc   = "started $($startDt.ToString('yyyy-MM-dd')), max 35 days"
                    } else {
                        $active = $true
                        $desc   = "start value '$startVal' unparseable - assuming active"
                    }
                }
                $scan.Details += ("{0}: {1}-update pause {2} ({3})" -f $psrc.Origin, $kind, $(if ($active) {"ACTIVE"} else {"expired"}), $desc)
                if ($active) {
                    $reason = "{0} ({1})" -f $psrc.Origin, $desc
                    if ($kind -eq 'Feature') {
                        $scan.FeatureActive  = $true
                        $scan.FeatureReasons += $reason
                    } else {
            ...
            $scan.Details += ("{0}: scan error - {1}" -f $psrc.Origin, "$_")
        }
    }
    return $scan
}

function Test-UpdatePauseState {
    Write-Log "=== Checking Windows Update Pause State ==="
    ...
    try {
        $scan = Get-UpdatePauseScan
        $pr.FeaturePaused = $scan.FeatureActive
        $pr.QualityPaused = $scan.QualityActive
        $pr.Details       = @($scan.Details)
        foreach ($d in @($scan.Details)) { Write-Log "  $d" }

        if ($scan.QualityActive) {
            $pr.Status = "WARN"
            $pr.Note   = "Quality updates are PAUSED - monthly security patching is suspended until the pause ends or is cleared (-ClearUpdatePauses)."
            Write-Log "  WARN: Quality updates are PAUSED - monthly patching is suspended." -Level "WARN"
            Set-Status "WARN"
        }
        if ($scan.FeatureActive) {
            $pr.Status = "WARN"
            $pr.Note   = ($pr.Note + " Feature updates are PAUSED - no feature update offers while active.").Trim()
            Write-Log "  WARN: Feature updates are PAUSED." -Level "WARN"
            Set-Status "WARN"
        }
        if (-not $scan.FeatureActive -and -not $scan.QualityActive) {
            $pr.Note = if (@($scan.Details).Count -gt 0) { "Expired/stale pause values present - not blocking." } else { "No pause values present." }
            Write-Log "  PASS: no active update pause."
        }
    } catch {
        $pr.Status = "ERROR"
        $pr.Note   = "Pause check failed: $_"
        Write-Log "  Update pause check failed: $_" -Level "WARN"
    }

    $Script:Results["UpdatePause"] = $pr
    return $pr
}
...
function Test-TimeSync {
    ...
    try {
        # v4.22: the Source line often carries a flags suffix (e.g.
        # "time.windows.com,0x9") that /stripchart cannot resolve - strip it.
        $ntpHost   = ($timeResult.NTPSource -split ',')[0].Trim()
        $stripText = (& w32tm.exe /stripchart /computer:$ntpHost `
...
#  Reports what is preventing a Windows 10/11 client below the
#  newest tracked release (currently 25H2) from receiving a feature
#  update offer. Skipped on Server SKUs and on hosts already at
#  25H2+ (build 26200 or higher).
#
#  Detects:
...
#    9. (v4.22) Pin CLASSIFICATION against the current product/build:
#       BLOCKING / TARGETING / INACTIVE / STALE / UNRECOGNISED.
#       Only BLOCKING and STALE pins are treated as blockers and
#       auto-removed under -UnblockFeatureUpdates; TARGETING pins
#       (set to drive an upgrade forward) are never touched.
#   10. (v4.22) Feature/quality update PAUSE state - WUfB policy key
#       AND Settings-app (UX\Settings) pause values
#   11. (v4.22) Legacy OS-upgrade blockers: DisableOSUpgrade,
#       AllowOSUpgrade=0, DeferUpgrade[Period]; NoAutoUpdate /
#       AUOptions reported (NoAutoUpdate=1 flagged, never removed)
#   12. (v4.22) Hardware appraiser verdicts (AppCompatFlags\
#       TargetVersionUpgradeExperienceIndicators: UpgEx / RedReason)
# ============================================================
# ----------------------------------------------------------------
#  Helper: classify a feature-update version pin (v4.22).
#  Compares TargetReleaseVersionInfo / ProductVersion against the
#  current product + build and returns one of:
#    NONE         - no pin configured
#    INACTIVE     - pin values present but the TargetReleaseVersion
#                   enable toggle is absent/0 (WU ignores the pin)
#    BLOCKING     - pin holds the host at its current version, or
#                   pins the product against a cross-product upgrade
#    STALE        - pin references a version/product BELOW current;
#                   suppresses offers until removed or auto-bumped
#                   at end of service
#    TARGETING    - pin points ABOVE the current version/product -
#                   it deliberately drives the upgrade; never remove
#    UNRECOGNISED - value does not map to a known release
# ----------------------------------------------------------------
function Get-FeatureUpdatePinClassification {
    param(
        [string]$PinnedVersion = "",
        [string]$PinnedProduct = "",
        $EnableToggle          = $null,
        [Parameter(Mandatory)] [int]$CurrentBuild
    )

    $mk = { param($c, $d) [pscustomobject]@{ Class = $c; Detail = $d } }

    $win10Map = @{ '1507'=10240; '1511'=10586; '1607'=14393; '1703'=15063; '1709'=16299;
                   '1803'=17134; '1809'=17763; '1903'=18362; '1909'=18363; '2004'=19041;
                   '20H2'=19042; '21H1'=19043; '21H2'=19044; '22H2'=19045 }
    $win11Map = @{ '21H2'=22000; '22H2'=22621; '23H2'=22631; '24H2'=26100; '25H2'=26200 }

    if (-not $PinnedVersion -and -not $PinnedProduct) {
        return (& $mk 'NONE' 'no version pin configured')
    }

    $currentProduct = if ($CurrentBuild -ge 22000) { 'Windows 11' } else { 'Windows 10' }

    # The pin only takes effect when the TargetReleaseVersion DWORD = 1
    if ($null -eq $EnableToggle) {
        return (& $mk 'INACTIVE' ("pin values present (TargetReleaseVersionInfo='{0}', ProductVersion='{1}') but the TargetReleaseVersion enable toggle is absent - Windows Update ignores the pin" -f $PinnedVersion, $PinnedProduct))
    }
    if ([int]$EnableToggle -ne 1) {
        return (& $mk 'INACTIVE' ("pin values present but TargetReleaseVersion={0} (must be 1) - Windows Update ignores the pin" -f [int]$EnableToggle))
    }

    # Resolve pinned product; defaults to the current product when not set
    $prodPin = $currentProduct
    if ($PinnedProduct) {
        if     ($PinnedProduct -match '11') { $prodPin = 'Windows 11' }
        elseif ($PinnedProduct -match '10') { $prodPin = 'Windows 10' }
        else { return (& $mk 'UNRECOGNISED' ("ProductVersion '{0}' is not a recognised product - review manually" -f $PinnedProduct)) }
    }

    # Product-only pin (no TargetReleaseVersionInfo)
    if (-not $PinnedVersion) {
        if ($prodPin -eq $currentProduct) {
            if ($currentProduct -eq 'Windows 10') {
                return (& $mk 'BLOCKING' 'ProductVersion pins host to Windows 10 - blocks the Windows 11 upgrade (Windows 10 feature updates still allowed)')
            }
            return (& $mk 'BLOCKING' 'ProductVersion pins host to Windows 11 - blocks any future cross-product upgrade (Windows 11 feature updates still allowed)')
        }
        if ($prodPin -eq 'Windows 11' -and $currentProduct -eq 'Windows 10') {
            return (& $mk 'TARGETING' 'ProductVersion=Windows 11 on a Windows 10 host - drives the Windows 11 upgrade; leave in place')
        }
        return (& $mk 'STALE' ("ProductVersion={0} but host already runs {1} - stale; suppresses offers, remove" -f $prodPin, $currentProduct))
    }

    # Version pin - map the release name to a build for the pinned product
    $map    = if ($prodPin -eq 'Windows 11') { $win11Map } else { $win10Map }
    $verKey = $PinnedVersion.Trim()
    if (-not $map.ContainsKey($verKey)) {
        return (& $mk 'UNRECOGNISED' ("TargetReleaseVersionInfo '{0}' is not a recognised {1} release - review manually" -f $PinnedVersion, $prodPin))
    }
    $pinBuild = [int]$map[$verKey]

    if ($prodPin -ne $currentProduct) {
        if ($prodPin -eq 'Windows 11' -and $currentProduct -eq 'Windows 10') {
            return (& $mk 'TARGETING' ("pin targets {0} {1} (build {2}) from {3} - drives the upgrade; leave in place" -f $prodPin, $PinnedVersion, $pinBuild, $currentProduct))
        }
        return (& $mk 'STALE' ("pin targets {0} {1} but host already runs {2} - stale; suppresses offers, remove" -f $prodPin, $PinnedVersion, $currentProduct))
    }

    if ($pinBuild -gt $CurrentBuild) {
        return (& $mk 'TARGETING' ("pin targets {0} {1} (build {2}), above current build {3} - drives the upgrade; leave in place" -f $prodPin, $PinnedVersion, $pinBuild, $CurrentBuild))
    }
    if ($pinBuild -eq $CurrentBuild) {
        return (& $mk 'BLOCKING' ("pin holds host at {0} {1} (current version) - blocks all newer feature updates" -f $prodPin, $PinnedVersion))
    }
    return (& $mk 'STALE' ("pin targets {0} {1} (build {2}), BELOW current build {3} - suppresses new offers until removed or auto-bumped at end of service" -f $prodPin, $PinnedVersion, $pinBuild, $CurrentBuild))
}
...
function Test-FeatureUpdateReadiness {
    ...
    $r = [ordered]@{
        ...
        PinClass                    = "NONE"
        PinDetail                   = ""
        PauseFeatureActive          = $null
        PauseQualityActive          = $null
        PauseDetails                = @()
        LegacyUpgradeBlockers       = @()
        NoAutoUpdate                = $null
        AUOptions                   = $null
        AppraiserBlocked            = $null
        AppraiserFindings           = @()
        BlockingReasons             = @()
        Status                      = "SKIP"
    }
    ...
    try {
        ...
        if ($build -ge 26200) {
            Write-Log "  Skipping - already on $rv (newest tracked release); feature update readiness check not relevant"
            $Script:Results["FeatureUpdateReadiness"] = $r
            return $r
        }
    ...
    try {
        ...
        if ($wu) {
            ...
            # v4.22: classify the pin against the current product/build
            # instead of flagging mere presence. Only BLOCKING / STALE /
            # UNRECOGNISED count as blockers. TARGETING (pin above the
            # current version - it drives the upgrade) and INACTIVE
            # (enable toggle absent/0) are reported but do not block.
            if ($r.TargetReleaseVersion -or $r.ProductVersion) {
                $pinnedVer  = if ($r.TargetReleaseVersion) { "$($r.TargetReleaseVersion)" } else { "" }
                $pinnedProd = if ($r.ProductVersion)       { "$($r.ProductVersion)" }       else { "" }
                $pinInfo = Get-FeatureUpdatePinClassification -PinnedVersion $pinnedVer -PinnedProduct $pinnedProd `
                               -EnableToggle $r.TargetReleaseVersionEnabled -CurrentBuild $build
                $r.PinClass  = $pinInfo.Class
                $r.PinDetail = $pinInfo.Detail
                Write-Log ("  Version pin classification: {0} - {1}" -f $pinInfo.Class, $pinInfo.Detail)
                if (@('BLOCKING','STALE','UNRECOGNISED') -contains $pinInfo.Class) {
                    $r.BlockingReasons += ("Version pin {0}: {1}" -f $pinInfo.Class, $pinInfo.Detail)
                }
            }
    ...
    # ---- (10) v4.22: feature/quality update PAUSE state ----
    # Shared scan (Get-UpdatePauseScan). The always-on Phase 1 check
    # (Test-UpdatePauseState) owns quality-pause WARNs on every SKU;
    # here only an active FEATURE pause is a readiness blocker.
    try {
        $pauseScan = Get-UpdatePauseScan
        $r.PauseFeatureActive = $pauseScan.FeatureActive
        $r.PauseQualityActive = $pauseScan.QualityActive
        $r.PauseDetails       = @($pauseScan.Details)
        if ($pauseScan.FeatureActive) {
            $r.BlockingReasons += ("Feature updates are PAUSED: {0}" -f (@($pauseScan.FeatureReasons) -join '; '))
        }
    } catch {
        Write-Log "  Pause-state check failed: $_" -Level "WARN"
    }

    # ---- (11) v4.22: legacy OS-upgrade block keys + AU policy ----
    try {
        $wuPol = Get-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate" -ErrorAction SilentlyContinue
        if ($wuPol) {
            if ((& $hasProp $wuPol 'DisableOSUpgrade') -and ([int]$wuPol.DisableOSUpgrade -eq 1)) {
                $r.LegacyUpgradeBlockers += 'DisableOSUpgrade=1 (Policies\WindowsUpdate)'
            }
            foreach ($legacyName in 'DeferUpgrade','DeferUpgradePeriod') {
                if ((& $hasProp $wuPol $legacyName) -and $wuPol.$legacyName) {
                    $r.LegacyUpgradeBlockers += ("{0}={1} (legacy 1511-era upgrade deferral)" -f $legacyName, $wuPol.$legacyName)
                }
            }
        }
        $osu = Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\OSUpgrade" -ErrorAction SilentlyContinue
        if ($osu -and (& $hasProp $osu 'AllowOSUpgrade') -and ([int]$osu.AllowOSUpgrade -eq 0)) {
            $r.LegacyUpgradeBlockers += 'AllowOSUpgrade=0 (CurrentVersion\WindowsUpdate\OSUpgrade)'
        }
        if (@($r.LegacyUpgradeBlockers).Count -gt 0) {
            $r.BlockingReasons += ("Legacy OS-upgrade block key(s) present: {0}" -f (@($r.LegacyUpgradeBlockers) -join '; '))
        }
    } catch {
        Write-Log "  Legacy upgrade-blocker check failed: $_" -Level "WARN"
    }

    try {
        $auPol = Get-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU" -ErrorAction SilentlyContinue
        if ($auPol) {
            if (& $hasProp $auPol 'NoAutoUpdate') {
                $r.NoAutoUpdate = [int]$auPol.NoAutoUpdate
                if ($r.NoAutoUpdate -eq 1) {
                    # v4.22: informational, NOT a blocking reason. Under
                    # N-central PME (which deploys this script), NoAutoUpdate=1
                    # is the expected managed state - PME installs patches via
                    # the WUA API, which this value does not block. Genuine
                    # neglect still surfaces through the LCU staleness check.
                    Write-Log "  NoAutoUpdate=1 - built-in AU scheduler disabled (expected under PME/RMM patch management; not counted as a blocker)"
                }
            }
            if (& $hasProp $auPol 'AUOptions') { $r.AUOptions = [int]$auPol.AUOptions }
        }
    } catch {
        Write-Log "  AU policy check failed: $_" -Level "WARN"
    }

    # ---- (12) v4.22: hardware appraiser verdicts ----
    # Setup's compatibility appraiser records per-target-release verdicts;
    # UpgEx=Red / RedReason lists the exact hardware blocks (TPM, CPU, ...).
    try {
        $appKey = "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\AppCompatFlags\TargetVersionUpgradeExperienceIndicators"
        if (Test-Path $appKey) {
            $r.AppraiserBlocked = $false
            foreach ($sub in (Get-ChildItem -Path $appKey -ErrorAction SilentlyContinue)) {
                $v = Get-ItemProperty -Path $sub.PSPath -ErrorAction SilentlyContinue
                if (-not $v) { continue }
                $upgEx     = if ((& $hasProp $v 'UpgEx')     -and $v.UpgEx)     { "$($v.UpgEx)" }     else { "" }
                $redReason = if ((& $hasProp $v 'RedReason') -and $v.RedReason) { "$($v.RedReason)" } else { "" }
                if (-not $upgEx -and -not $redReason) { continue }
                $r.AppraiserFindings += ("{0}: UpgEx={1}{2}" -f $sub.PSChildName, $(if ($upgEx) {$upgEx} else {"?"}), $(if ($redReason -and $redReason -ne 'None') {", RedReason=$redReason"} else {""}))
                if ($upgEx -eq 'Red' -or ($redReason -and $redReason -ne 'None')) { $r.AppraiserBlocked = $true }
            }
            if ($r.AppraiserBlocked) {
                $r.BlockingReasons += ("Hardware appraiser marks an upgrade target RED: {0}" -f (@($r.AppraiserFindings) -join ' | '))
            }
        }
    } catch {
        Write-Log "  Appraiser verdict check failed: $_" -Level "WARN"
    }
...
# (v4.25: the pre-v4.3 Invoke-CapturedCommand helper was removed - it had
# no remaining callers; all captured execution goes through
# Invoke-CapturedCommandWithTimeout.)
...
function Get-DISMErrorMeaning {
    ...
    switch ($HResult.ToLower()) {
        ...
        '0x800f0915' { 'Repair source content not found (common on 24H2/25H2) - usually an outdated servicing stack on a patch-stale host; install the latest CU first, or rerun RestoreHealth with /Source from matching media' }
...
function Invoke-DiskCleanupRemediation {
    ...
    # v4.22: StartComponentCleanup refuses (0x800f0bc9) when a serious
    # reboot is pending - skip it (the temp prune below still runs).
    $rebP = $Script:Results["PendingReboot"]
    if (-not $cleanupSkipReason -and $rebP -and $rebP["RebootPending"] -and -not $ForceCleanup) {
        $cleanupSkipReason = "serious reboot pending; StartComponentCleanup refuses with staged operations"
    }
...
function Invoke-WUComponentReset {
    ...
    # v4.24: bookkeeping only triggers a reset on the true corruption
    # signature - WUA COM sees LCU history while BOTH legacy lookups
    # return zero KBs. Probe timeouts (-1 = Unknown) and empty-WUA cases
    # do NOT trigger: a reset cannot fix a broken WMI provider, and on an
    # empty-history box it only destroys the history still rebuilding.
    $bookkeepingCorrupt = ($lcu -and $lcu.Contains("BookkeepingHealthy") -and -not $lcu["BookkeepingHealthy"] -and
                           ($lcu["HotFixCount"] -eq 0) -and ($lcu["WMICount"] -eq 0) -and
                           ([int]$lcu["TotalLCUsFound"] -gt 0))
    if ($bookkeepingCorrupt) {
        ...
        if (-not (Test-RecentSoftwareDistributionReset -WithinHours 24)) {
            $reasons += "patch bookkeeping CORRUPT (WUA sees history; Get-HotFix and WMI both return zero KBs)"
        } else {
    ...
    # v4.22: the 24h recent-reset guard now applies to ALL trigger reasons,
    # not just degraded bookkeeping. Re-renaming SoftwareDistribution every
    # run (e.g. on a host whose WU services are Disabled by policy)
    # destroys history and in-flight downloads without fixing anything.
    if (-not $ForceComponentReset -and (Test-RecentSoftwareDistributionReset -WithinHours 24)) {
        Add-Remediation -Step "WU Component Reset" -Result "SKIPPED" `
            -Detail ("triggered by: {0} - but a SoftwareDistribution.bak-* under 24h old exists; letting the prior reset settle (-ForceComponentReset overrides)" -f ($reasons -join '; '))
        return
    }
    ...
    # v4.22: usosvc + dosvc added - both can hold handles inside
    # SoftwareDistribution and make the rename fail with 'in use'.
    $stopServices = @('usosvc', 'dosvc', 'wuauserv', 'bits', 'cryptsvc', 'msiserver', 'appidsvc')
    foreach ($n in $stopServices) {
        try {
            $s = Get-Service -Name $n -ErrorAction SilentlyContinue
            if (-not $s) { continue }
            if ($s.Status -ne 'Stopped') {
                Stop-Service -Name $n -Force -WarningAction SilentlyContinue -ErrorAction Stop
    ...
    $startServices = @('cryptsvc', 'bits', 'msiserver', 'appidsvc', 'wuauserv', 'dosvc', 'usosvc')
    foreach ($n in $startServices) {
        try {
            ...
            if ($s.Status -ne 'Running') {
                Start-Service -Name $n -WarningAction SilentlyContinue -ErrorAction Stop
    ...
    # ---- Step 7 (v4.25): .bak retention prune moved to MAIN Phase 2 -
    # it now runs on every run, not only after a reset. ----
}

# V4 helper: clean up SoftwareDistribution.bak-* and catroot2.bak-*
# folders older than $BakRetentionDays. v4.25: runs on every Phase 2
# pass - previously it only ran after a component reset, so healthy
# hosts never pruned old multi-GB backups.
...
# ----------------------------------------------------------------
#  Update pause clearing - GATED (v4.22). Runs on any SKU. Only
#  acts when a pause is ACTIVE and the operator asked for clearing
#  via -ClearUpdatePauses (or -UnblockFeatureUpdates, which implies
#  it for the feature-update flow).
# ----------------------------------------------------------------
function Invoke-UpdatePauseRemediation {
    Write-Log "=== Remediation: Update pause state ==="

    $up = $Script:Results["UpdatePause"]
    $anyActive = ($up -and (($up["FeaturePaused"] -eq $true) -or ($up["QualityPaused"] -eq $true)))
    if (-not $anyActive) {
        Add-Remediation -Step "Clear update pauses" -Result "SKIPPED" -Detail "no active pause detected"
        return
    }
    if (-not ($ClearUpdatePauses -or $UnblockFeatureUpdates)) {
        Add-Remediation -Step "Clear update pauses" -Result "SKIPPED" `
            -Detail "active pause detected but neither -ClearUpdatePauses nor -UnblockFeatureUpdates specified (pauses may be deliberate, e.g. dodging a bad KB)"
        return
    }

    Ensure-WURegistryBackup

    $pauseVals = @('PauseFeatureUpdatesStartTime','PauseFeatureUpdatesEndTime',
                   'PauseQualityUpdatesStartTime','PauseQualityUpdatesEndTime',
                   'PauseUpdatesExpiryTime','PauseUpdatesStartTime',
                   'PausedFeatureStatus','PausedQualityStatus')
    $pauseKeys = @("HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate",
                   "HKLM:\SOFTWARE\Microsoft\WindowsUpdate\UX\Settings")
    $cleared = 0
    foreach ($pKey in $pauseKeys) {
        if (-not (Test-Path $pKey)) { continue }
        foreach ($pv in $pauseVals) {
            try {
                if (Get-ItemProperty -Path $pKey -Name $pv -ErrorAction SilentlyContinue) {
                    Remove-ItemProperty -Path $pKey -Name $pv -ErrorAction Stop
                    $cleared++
                    Add-Remediation -Step "Clear update pause: $pv" -Result "OK" -Detail ($pKey -replace '^HKLM:\\SOFTWARE\\','')
                }
            } catch {
                Add-Remediation -Step "Clear update pause: $pv" -Result "FAILED" -ErrorMessage "$_"
            }
        }
    }
    if ($Script:Environment.WsusManaged) {
        Write-Log "  Note: host is WSUS/PME-managed - a domain GPO may re-apply policy pause values at next refresh." -Level "INFO"
    }
    if ($cleared -eq 0) {
        Add-Remediation -Step "Clear update pauses" -Result "INFO" -Detail "no pause values found to remove"
    }
}
...
function Invoke-FeatureUpdateReadinessRemediation {
        ...
        foreach ($a in $stale) {
            ...
            try {
                ...
                try {
                    ...
                    # v4.22: upgrade artifacts are TrustedInstaller-owned; take
                    # ownership (Administrators) and retry once before failing.
                    Write-Log "  Direct delete failed; taking ownership and retrying..." -Level "WARN"
                    & takeown.exe /F "$($a['Path'])" /R /A /D Y 2>&1 | Out-Null
                    & icacls.exe "$($a['Path'])" /grant "*S-1-5-32-544:(OI)(CI)F" /T /C /Q 2>&1 | Out-Null
                    Remove-Item -LiteralPath $a["Path"] -Recurse -Force -ErrorAction Stop
                }
    ...
    # v4.22: pins are only actionable when classified BLOCKING or STALE.
    # TARGETING pins (deliberately driving an upgrade) and INACTIVE pins
    # are never removed; UNRECOGNISED pins require manual review.
    $pinActionable  = (@("BLOCKING","STALE") -contains $r["PinClass"])
    $pauseActive    = (($r["PauseFeatureActive"] -eq $true) -or ($r["PauseQualityActive"] -eq $true))
    $legacyPresent  = (@($r["LegacyUpgradeBlockers"]).Count -gt 0)
    $hasPolicyBlocker = ($pinActionable -or $r["DeferFeatureUpdates"] -or
                         ($r["AllowTelemetry"] -eq 0) -or $pauseActive -or $legacyPresent)

    if (-not $UnblockFeatureUpdates) {
        if ($hasPolicyBlocker) {
            Add-Remediation -Step "Feature update policy unblock" -Result "SKIPPED" `
                -Detail "policy/telemetry/pause changes require -UnblockFeatureUpdates (pauses alone: -ClearUpdatePauses); script does not touch WU policy keys by default"
        }
        return
    }
    ...
    # v4.22: only BLOCKING / STALE pins are removed. Everything else is
    # surfaced but deliberately left alone.
    switch ($r["PinClass"]) {
        "TARGETING"    { Add-Remediation -Step "Feature update version pin" -Result "INFO"    -Detail ("classified TARGETING - deliberately drives the upgrade; left in place ({0})" -f $r["PinDetail"]) }
        "INACTIVE"     { Add-Remediation -Step "Feature update version pin" -Result "INFO"    -Detail ("classified INACTIVE - enable toggle not set, not blocking; left in place ({0})" -f $r["PinDetail"]) }
        "UNRECOGNISED" { Add-Remediation -Step "Feature update version pin" -Result "SKIPPED" -Detail ("classified UNRECOGNISED - not auto-removed; review manually ({0})" -f $r["PinDetail"]) }
    }
    if ($pinActionable) {
        foreach ($val in 'TargetReleaseVersion','TargetReleaseVersionInfo','ProductVersion') {
            try {
                ...
                if ($null -ne $existing) {
                    ...
                    Add-Remediation -Step "Remove WU policy: $val" -Result "OK" -Detail ("feature-update pin cleared (was classified {0})" -f $r["PinClass"])
                }
            } catch {
    ...
    # v4.22: pause clearing moved to Invoke-UpdatePauseRemediation
    # (always-on check owns it; gated on -ClearUpdatePauses /
    # -UnblockFeatureUpdates).

    # v4.22: remove legacy OS-upgrade blockers (NoAutoUpdate is never touched)
    if ($legacyPresent) {
        foreach ($lv in 'DisableOSUpgrade','DeferUpgrade','DeferUpgradePeriod') {
            try {
                if (Get-ItemProperty -Path $wuKey -Name $lv -ErrorAction SilentlyContinue) {
                    Remove-ItemProperty -Path $wuKey -Name $lv -ErrorAction Stop
                    Add-Remediation -Step "Remove legacy blocker: $lv" -Result "OK"
                }
            } catch {
                Add-Remediation -Step "Remove legacy blocker: $lv" -Result "FAILED" -ErrorMessage "$_"
            }
        }
        try {
            $osuKey = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\OSUpgrade"
            $osu = Get-ItemProperty -Path $osuKey -Name AllowOSUpgrade -ErrorAction SilentlyContinue
            if ($osu -and ([int]$osu.AllowOSUpgrade -eq 0)) {
                Set-ItemProperty -Path $osuKey -Name AllowOSUpgrade -Value 1 -Type DWord -ErrorAction Stop
                Add-Remediation -Step "Set AllowOSUpgrade=1" -Result "OK" -Detail "was 0"
            }
        } catch {
            Add-Remediation -Step "Set AllowOSUpgrade=1" -Result "FAILED" -ErrorMessage "$_"
        }
    }
...
function Invoke-PendingRebootRemediation {
    ...
    try {
        ...
        # v4.22: only rename when the machine has REBOOTED since pending.xml
        # was written and the file is still here - proof the merge is stuck.
        # If no reboot has happened since it was written, the correct fix is
        # the reboot itself; renaming a live pending transaction can corrupt
        # component servicing.
        $lastBoot = $null
        try { $lastBoot = (Get-CimInstance -ClassName Win32_OperatingSystem -ErrorAction Stop).LastBootUpTime } catch {}
        $fileTime = (Get-Item $pending).LastWriteTime
        if (-not $lastBoot -or $lastBoot -le $fileTime) {
            Add-Remediation -Step "pending.xml age check" -Result "SKIPPED" `
                -Detail ("{0:N1} days old but no reboot since it was written - reboot first; rename only if it survives a reboot" -f $age.TotalDays)
            return
        }
        $stamp   = Get-Date -Format "yyyyMMdd-HHmmss"
...
function Invoke-WUDetection {
    ...
    # v4.22: never scan/install via the online Microsoft Update service on
    # WSUS/PME-managed hosts - -MicrosoftUpdate registers the MU service
    # and bypasses upstream approvals. Managed hosts scan their configured
    # service instead.
    $useMU = -not ($Script:Environment.WsusManaged -eq $true)
    ...
    if ($Script:Environment.HasPSWindowsUpdate) {
        try {
            ...
            $start = Get-Date
            $list = if ($useMU) { Get-WUList -MicrosoftUpdate -ErrorAction Stop }
                    else        { Get-WUList -ErrorAction Stop }
            $count = if ($list) { @($list).Count } else { 0 }
            $dur = ((Get-Date) - $start).TotalSeconds
            $usedPSWU = $true
            Add-Remediation -Step "Force WU scan (PSWindowsUpdate Get-WUList)" -Result "OK" -DurationSec $dur -Detail ("$count update(s) currently applicable" + $(if (-not $useMU) {" (via configured WSUS/PME service)"} else {""}))
        } catch {
            $wuErr = "$_"
            # v4.25: this WUA search error is typically transient while the
            # WU datastore is rebuilding (fresh SoftwareDistribution) and
            # clears after a reboot or on the next run. The UsoClient
            # fallback below still triggers a background scan.
            if ($wuErr -match 'Value does not fall within the expected range') {
                $wuErr += " (typically transient while the WU datastore rebuilds; clears after reboot/next run)"
            }
            Add-Remediation -Step "Force WU scan (PSWindowsUpdate Get-WUList)" -Result "FAILED" -ErrorMessage $wuErr
        }
    }
        ...
        if (Test-Path $uso) {
            try {
                ...
                Add-Remediation -Step "Force WU scan (UsoClient StartScan)" -Result $(if ($p.ExitCode -eq 0) {"OK"} else {"INFO"}) `
                    -DurationSec $dur -Detail "exit $($p.ExitCode) (async trigger; scan completes in background)"
            } catch {
    ...
    # v4.22: never install on top of a serious pending reboot
    # (0x80242014 WU_E_UH_POSTREBOOTSTILLPENDING risk).
    $rebP = $Script:Results["PendingReboot"]
    if ($InstallApplicable -and $rebP -and $rebP["RebootPending"]) {
        Add-Remediation -Step "Install applicable updates (-InstallApplicable)" -Result "SKIPPED" `
            -Detail "serious reboot pending - reboot before installing further updates"
    } elseif ($InstallApplicable -and $usedPSWU -and $count -gt 0) {
        ...
        try {
            ...
            # v4.25: exclude the "Upgrades" category unless -AllowFeatureUpdate
            # was given - AcceptAll must never stage an OS version change
            # (e.g. the Windows 11 feature update) as a side effect.
            $installParams = @{
                AcceptAll    = $true
                IgnoreReboot = $true
                Confirm      = $false
            ...
            if ($useMU) { $installParams['MicrosoftUpdate'] = $true }
            if (-not $AllowFeatureUpdate) { $installParams['NotCategory'] = @('Upgrades') }
            ...
            Add-Remediation -Step "Install applicable updates (-InstallApplicable)" -Result "OK" -DurationSec $dur `
                -Detail ("$installCount update(s) processed via PSWindowsUpdate; reboot may be required" + $(if ($AllowFeatureUpdate) {" (feature updates ALLOWED)"} else {" (Upgrades category excluded)"}))
        } catch {
...
function Invoke-PostRemediationRecheck {
    ...
    # v4.22: re-check update pause state if the initial scan flagged it
    $ups = $Script:PreCheckSnapshot["UpdatePause"]
    if ($ups -and ($ups["Status"] -in @("WARN","FAIL","ERROR"))) {
        $candidates["UpdatePause"] = { Test-UpdatePauseState }
    }

    if ($candidates.Count -eq 0) {
        # v4.22: outcome NONE (not REMEDIATED) - nothing was remediated, and
        # any residual WARN from non-recheckable checks (uptime, TPM, EOL,
        # Fast Startup) stays in the final status / exit code.
        Write-Log "  No re-checkable items were failing in the initial scan."
        $Script:RemediationOutcome = "NONE"
        return
    }
    ...
    # v4.22: seed the post-remediation status with the WARN/FAIL floor
    # contributed by checks Phase 3 never re-runs (uptime, TPM, EOL/ESU
    # lifecycle, Fast Startup). Previously the flat PASS reset silently
    # dropped these from the final status and exit code.
    $floor = "PASS"
    $snapUp  = if ($Script:PreCheckSnapshot.Contains("Uptime"))      { $Script:PreCheckSnapshot["Uptime"] }      else { $null }
    $snapTpm = if ($Script:PreCheckSnapshot.Contains("TPM"))         { $Script:PreCheckSnapshot["TPM"] }         else { $null }
    $snapEol = if ($Script:PreCheckSnapshot.Contains("EOL"))         { $Script:PreCheckSnapshot["EOL"] }         else { $null }
    $snapFs  = if ($Script:PreCheckSnapshot.Contains("FastStartup")) { $Script:PreCheckSnapshot["FastStartup"] } else { $null }
    if ($snapUp  -and ($snapUp["Status"]  -in @("WARN","FAIL"))) { $floor = "WARN" }
    if ($snapTpm -and  $snapTpm["Status"] -eq "WARN")            { $floor = "WARN" }
    if ($snapFs  -and  $snapFs["Status"]  -eq "WARN")            { $floor = "WARN" }
    if ($snapEol) {
        if     ($snapEol["IsEOL"]) { $floor = "FAIL" }
        elseif (($snapEol["RawEOL"] -and $snapEol["ESUOverride"]) -or ("$($snapEol['EndOfSupportDate'])" -eq "Unknown")) { if ($floor -ne "FAIL") { $floor = "WARN" } }
        elseif (($snapEol["DaysUntilEOL"] -is [int]) -and ($snapEol["DaysUntilEOL"] -le 90)) { if ($floor -ne "FAIL") { $floor = "WARN" } }
    }
    if ($floor -ne "PASS") {
        Write-Log ("  Non-recheckable checks (uptime/TPM/EOL/FastStartup) carry a residual {0} into the final status." -f $floor)
    }
    $Script:OverallStatus = $floor

    foreach ($key in $candidates.Keys) {
        try {
            Write-Log "  -> Re-running $key"
            # v4.22: capture the returned result dict - uncaptured it leaks
            # into the console output as a raw Name/Value table.
            $null = & $candidates[$key]
        } catch {
    ...
    foreach ($key in $candidates.Keys) {
        ...
        $isFailing = switch ($key) {
            ...
            # v4.22: was missing - FUR could never count as still-failing,
            # inflating the outcome to REMEDIATED.
            "FeatureUpdateReadiness" { $now -and ($now["Status"] -in @("WARN","FAIL","ERROR")) }
            "UpdatePause"   { $now -and (($now["FeaturePaused"] -eq $true) -or ($now["QualityPaused"] -eq $true)) }
        }

        if ($isFailing) {
            # v4.22 demote: FUR blockers need -UnblockFeatureUpdates to be
            # remediable; without the switch, leave them as a WARN in the
            # final status instead of failing the whole run.
            if ($key -eq "FeatureUpdateReadiness" -and -not $UnblockFeatureUpdates) {
                Write-Log "  FeatureUpdateReadiness re-check: blockers remain but -UnblockFeatureUpdates was not requested. Leaving as WARN." -Level "INFO"
                continue
            }

            # v4.22: pauses can only be cleared with -ClearUpdatePauses /
            # -UnblockFeatureUpdates; without them, leave as WARN.
            if ($key -eq "UpdatePause" -and -not ($ClearUpdatePauses -or $UnblockFeatureUpdates)) {
                Write-Log "  UpdatePause re-check: pause remains but clearing was not requested (-ClearUpdatePauses). Leaving as WARN." -Level "INFO"
                continue
            }
            ...
            # v4.10/v4.22 demote: ANY serious pending source (CBS, WU, SCCM,
            # domain-join) only clears on an actual reboot, which this script
            # never performs by design. Flag REBOOT_REQUIRED instead of
            # counting it as a remediation failure.
            if ($key -eq "PendingReboot" -and $now -and $now["RebootPending"]) {
                $sources = if ($now.Contains("Sources")) { @($now["Sources"]) } else { @() }
                Write-Log "  PendingReboot re-check: still pending ($($sources -join ', ')). Not script-fixable - reboot during patch maintenance window will clear it. Flagging run as REBOOT_REQUIRED." -Level "INFO"
    ...
    if ($stillFailing.Count -eq 0) {
        # v4.22: demoted items (LCU deferred, FUR/pause without their
        # switch) re-applied WARN/FAIL via Set-Status when re-run. They
        # are deferred-not-failed, so cap the final status at WARN unless
        # the non-recheckable floor itself is FAIL (e.g. EOL, no ESU).
        if ($Script:OverallStatus -eq "FAIL" -and $floor -ne "FAIL") {
            Write-Log "  Only demoted (deferred) items re-applied FAIL during re-check; capping final status at WARN."
...
function Write-ComplianceReport {
    ...
    # ---- Update Pause State (v4.22) ----
    $upause = $AllResults["UpdatePause"]
    Header "WINDOWS UPDATE PAUSE STATE"
    if ($upause) {
        $qFlag = if ($upause["QualityPaused"]) { "!! MONTHLY PATCHING SUSPENDED !!" } else { "" }
        $fFlag = if ($upause["FeaturePaused"]) { "FEATURE UPDATE OFFERS BLOCKED" } else { "" }
        Row "Quality updates paused"  $(if ($upause["QualityPaused"]) {"YES"} else {"No"}) $qFlag
        Row "Feature updates paused"  $(if ($upause["FeaturePaused"]) {"YES"} else {"No"}) $fFlag
        foreach ($pd in @($upause["Details"])) { Write-Output ("      - {0}" -f $pd) }
        Row "Status"                  $upause["Status"] $qFlag
        Row "Note"                    $upause["Note"]
    } else {
        Write-Output "  [Update pause check not performed]"
    }
    ...
    if ($reboot) {
        ...
        Row "PFR Note"                "PFR entries are normal OS behaviour and do not gate remediation."
    } else {
    ...
    # ---- Fast Startup (v4.22) ----
    $fastb = $AllResults["FastStartup"]
    Header "FAST STARTUP (hiberboot)"
    if ($fastb) {
        if (-not $fastb["Applicable"]) {
            Write-Output "  Not applicable - $($fastb['Note'])"
        } else {
            $fsFlag = if ($fastb["Status"] -eq "WARN") { "SHUTDOWNS ARE NOT FINALISING PATCHES" } else { "" }
            Row "Fast Startup Enabled" $(if ($fastb["Enabled"]) {"Yes"} elseif ($null -eq $fastb["Enabled"]) {"Unknown"} else {"No"}) $fsFlag
            Row "Status"               $fastb["Status"] $fsFlag
            Row "Note"                 $fastb["Note"]
        }
    } else {
        Write-Output "  [Fast Startup check not performed]"
    }
    ...
    if ($fur -and $fur["Applicable"]) {
        ...
        if ($fur["PinClass"] -and $fur["PinClass"] -ne "NONE") {
            $pinFlag = switch ($fur["PinClass"]) {
                "BLOCKING"     { "!! BLOCKS FEATURE UPDATES !!" }
                "STALE"        { "STALE PIN - REMOVE" }
                "UNRECOGNISED" { "REVIEW MANUALLY" }
                "TARGETING"    { "DRIVES UPGRADE - LEAVE IN PLACE" }
                default        { "" }
            }
            Row "Pin classification"        $fur["PinClass"]  $pinFlag
            Row "  Pin analysis"            $fur["PinDetail"]
        }

        $pfDisp = if ($null -eq $fur["PauseFeatureActive"]) { "(not checked)" } elseif ($fur["PauseFeatureActive"]) { "YES - PAUSED (see WINDOWS UPDATE PAUSE STATE section)" } else { "No" }
        Row "Feature updates paused"    $pfDisp  $(if ($fur["PauseFeatureActive"]) {"BLOCKS FEATURE UPDATE OFFERS"})

        if (@($fur["LegacyUpgradeBlockers"]).Count -gt 0) {
            Row "Legacy upgrade blockers"   (@($fur["LegacyUpgradeBlockers"]) -join " | ")  "REMOVE (or use -UnblockFeatureUpdates)"
        } else {
            Row "Legacy upgrade blockers"   "(none)"
        }

        $naDisp = if ($null -eq $fur["NoAutoUpdate"]) { "(not set)" } else { $fur["NoAutoUpdate"] }
        $auDisp = if ($null -ne $fur["AUOptions"]) {
            $auMeaning = switch ([int]$fur["AUOptions"]) {
                1       { "automatic updates disabled" }
                2       { "notify before download" }
                3       { "auto-download, notify install" }
                4       { "auto-download, scheduled install" }
                5       { "local admin chooses" }
                7       { "auto-download, notify install+restart" }
                default { "unknown value" }
            }
            "$($fur['AUOptions']) ($auMeaning)"
        } else { "(not set)" }
        Row "AU: NoAutoUpdate / AUOptions"  ("{0} / {1}" -f $naDisp, $auDisp)  $(if ($fur["NoAutoUpdate"] -eq 1) {"AU DISABLED - expected under PME/RMM management"})
        ...
        if ($null -ne $fur["AppraiserBlocked"]) {
            Row "Hardware appraiser"        $(if ($fur["AppraiserBlocked"]) {"RED - upgrade blocked by hardware"} else {"Green - no hardware blocks recorded"})  $(if ($fur["AppraiserBlocked"]) {"!! HARDWARE BLOCK !!"})
            foreach ($af in @($fur["AppraiserFindings"])) { Write-Output ("      - {0}" -f $af) }
        } else {
            Row "Hardware appraiser"        "(no appraiser verdicts recorded)"
        }
        ...
        if (@($fur["SafeguardSignals"]).Count -gt 0) {
            ...
            foreach ($s in $fur["SafeguardSignals"]) { Write-Output ("      - {0}" -f $s) }
        } else {
    ...
    if ($Script:PreCheckSnapshot.Count -eq 0) {
        ...
        Write-Output ("  {0,-24} {1,-30} {2,-30}" -f "Check","Before","After")
        Write-Output ("  " + ("-" * 80))
        ...
        function Get-StatusOf {
            ...
            switch ($Key) {
                ...
                "FeatureUpdateReadiness" { return ("{0} ({1} blocker(s))" -f $Result["Status"], @($Result["BlockingReasons"]).Count) }
                "UpdatePause"   { return $(if (($Result["FeaturePaused"] -eq $true) -or ($Result["QualityPaused"] -eq $true)) {"PAUSED"} else {"None"}) }
                default         { return "(n/a)" }
            }
        }

        foreach ($key in @("LCUCheck","DiskSpace","WUServices","PendingReboot","TimeSync","FeatureUpdateReadiness","UpdatePause")) {
            ...
            Write-Output ("  {0,-24} {1,-30} {2,-30}" -f $key, $b, $a)
        }
    }
...
Write-Log "############################################################"
if ($Script:UnlimitedBudget) {
    Write-Log " Time budget: UNLIMITED (no internal cap; N-central task timeout is the only ceiling)"
    Write-Log " (set -MaxRuntimeMinutes to a positive value to impose an internal deadline that guarantees the report ships)"
} else {
    Write-Log (" Time budget: {0} minutes (deadline {1} UTC)" -f $MaxRuntimeMinutes, $Script:DeadlineUtc.ToString('yyyy-MM-dd HH:mm:ss'))
    Write-Log " (configurable via -MaxRuntimeMinutes; 0 = unlimited)"
}
Write-Log "############################################################"
...
try {
    ...
    $fastBootInfo = Test-FastStartup
    $pauseState   = Test-UpdatePauseState
    ...
    # v4.22: a serious pending reboot (CBS/WU/SCCM/domain-join) gates the
    # destructive steps below. PFR-only pending never gates - PFR entries
    # exist on virtually every live system. Per-step Force switches override.
    $rebSnap = $Script:Results["PendingReboot"]
    $seriousRebootPending = [bool]($rebSnap -and $rebSnap["RebootPending"])
    if ($seriousRebootPending) {
        Write-Log ("  Serious pending reboot detected ({0}) - destructive steps will be gated; expect REBOOT_REQUIRED outcome." -f (@($rebSnap["Sources"]) -join ', ')) -Level "WARN"
    }
    ...
    # v4.22: gated on serious pending reboot - resetting components can
    # orphan an install that is staged awaiting reboot.
    if ($seriousRebootPending -and -not $ForceComponentReset) {
        Add-Remediation -Step "WU Component Reset" -Result "SKIPPED" `
            -Detail "serious reboot pending - reset could orphan staged updates (-ForceComponentReset to override)"
    } else {
        Invoke-IfBudget -Label "WU Component Reset" -NeededSec 240 -Block { Invoke-WUComponentReset }
    }

    # v4.25: .bak retention prune runs every Phase 2 pass (was previously
    # only reachable at the end of a component reset, so healthy hosts
    # kept old multi-GB SoftwareDistribution.bak-* folders forever).
    Invoke-IfBudget -Label "Prune .bak-* folders" -NeededSec 30 -Block { Invoke-BakRetentionPrune }
    ...
    # v4.22: update pause clearing (gated inside the function on
    # -ClearUpdatePauses / -UnblockFeatureUpdates).
    Invoke-IfBudget -Label "Clear update pauses" -NeededSec 30 -Block { Invoke-UpdatePauseRemediation }
    ...
    # v4.22: gated on serious pending reboot - DISM/SFC refuse or misreport
    # against a store with staged operations (0x800f0bc9).
    if ($seriousRebootPending -and -not ($ForceFullDISM -or $ForceSfc)) {
        Add-Remediation -Step "Component store repair (DISM + SFC)" -Result "SKIPPED" `
            -Detail "serious reboot pending - DISM/SFC refuse with staged operations; reboot first (-ForceFullDISM/-ForceSfc to override)"
    } else {
...
# Exit code mapping
# v4.22: REMEDIATED / REBOOT_REQUIRED no longer force exit 0 over a
# residual WARN/FAIL from checks remediation cannot fix (uptime, TPM,
# EOL/ESU, Fast Startup, policy pins). The exit code is the worse of the
# remediation outcome and the final overall status.
$statusExit = switch ($Script:OverallStatus) {
    "PASS" { 0 }
    "WARN" { 1 }
    "FAIL" { 2 }
    default { 1 }
}
$outcomeExit = switch ($Script:RemediationOutcome) {
    "REMEDIATED"      { 0 }
    "REBOOT_REQUIRED" { 0 }   # success - needs reboot; residual WARN still surfaces via status
    "NONE"            { 0 }
    "PARTIAL"         { 1 }
    "TRUNCATED"       { 1 }
    "FAIL"            { 2 }
...
$exit = [Math]::Max($statusExit, $outcomeExit)
