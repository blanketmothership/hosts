    ...
    if ($RunUpdateScan) {
        ...
        try {
            ...
            if ($result.Updates.Count -eq 0) {
                ...
                if ($featureOffered) {
                    Write-Log "FEATURE UPDATE OFFERED: $TargetProductVersion $TargetReleaseInfo is available on this device."
                }
                else {
                    Write-Log "Note: $TargetReleaseInfo feature update not offered yet (already on $TargetReleaseInfo, not yet released to this device, or blocked by a safeguard hold)."
                }
            }
        }
        catch {
