...
.NOTES
    Get-NCentralThirdPartyPatchReport_V2.4.ps1
    V2.4 - 2026-07-23 - polish from the first fully-healthy probe run:
        - Cross-source duplicate rows collapse better: product matching now
          strips arch tokens, so 'Firefox (x64) 152.0.6' (PME action) and
          'Mozilla Firefox 152.0.6' (PME 3PP) merge into one row.
        - The pointed probe-cache advice only prints when proxy timeouts are
          RECENT (<=7 days); older ones get a one-line historical note
          instead of present-tense alarm on a healthy device.
        - 'Notepad ++' (with space) classifies as 3P.
    V2.3 - 2026-07-23 - offline-blip prevention:
        - The script now runs at BelowNormal process priority (opt out with
          -NormalPriority): on slow hardware the old 20-56 min parse/zip
          could starve the N-central agent's check-ins long enough for the
          device to blip 'offline' in the console. Combined with V2.2's
          3-4x faster parsing, report runs should no longer be noticeable.
        - Remediation service handling is dependency-safe: services that
          Stop-Service -Force silently drags down are recorded and
          restarted, and a final safety net re-checks EVERY service that
          was Running at Step 1 (now including EcosystemAgent) and starts
          any that are not Running. The 'Windows Agent Service' itself is
          never stopped by this script - by design.
    V2.2 - 2026-07-23 - ALA-22534 run fixes:
        - PERFORMANCE: all PME component logs except ThirdPartyPatch*/
          ActionManager now use a cheap error/warn/config-line pre-filter
          before any regex. Win11 workstations were parsing at ~8-10s/MB
          (21-56 min Step 4); this cuts the bulk of that work.
        - Patch-cache metrics now read ONLY from FileCacheServiceAgent logs -
          the SendManager/CacheService components have their own small (100
          MB) caches whose config lines previously mixed into the health
          numbers ('11932 MB of 100 MB configured') and false-fired the
          near-cap warning.
        - NEW engine-error types counted from ActionManager: 'Approved patch
          missing from device metadata' (seen looping every 5 min for 4
          patch ids) and 'Approved patch not present in WUA catalog'. These
          also gate RECOVERY: catalogs downloading does not mean a
          metadata-desynced device is healthy.
        - 'A reboot is required to complete the install' action results are
          SUCCESS-REBOOT, not FAILED. ActionManager lines are excluded from
          the generic extraction (kills GUID-only ghost rows).
        - QueryManager 'couldn't map field' WARN spam filtered from evidence.
...
param(
    ...
    [switch]$NormalPriority,   # keep normal process priority (default: BelowNormal so the agent is never starved)
...
try {
    ...
    # run at BelowNormal priority so heavy parsing/zipping can never starve
    # the N-central agent's check-in cycle (a saturated box can otherwise
    # blip 'offline' in the console during long runs on slow hardware)
    if (-not $NormalPriority) {
        try {
            [System.Diagnostics.Process]::GetCurrentProcess().PriorityClass = [System.Diagnostics.ProcessPriorityClass]::BelowNormal
            Write-Log 'Process priority set to BelowNormal (agent check-ins always win; use -NormalPriority to disable).'
        } catch { }
    }
    ...
    $svcNames = @(
        ...
        'EcosystemAgent', 'EcosystemAgentMaintenance',
    ...
    $noiseRegex      = '(?i)(heartbeat|polling|GET\s+/|POST\s+/|keep.?alive|Successfully installed the file|downloadInfoDetails|RegOpenKeyEx|couldn.?t map field)'
    ...
    $newestDlErr    = $null    # newest DOWNLOAD-class engine error (for recovery detection)
    $dlLabels       = @('Catalog/package unzip failed',
                        'Catalog/package download failed (FileCache/CacheService)',
                        'Cache service unreachable (IPC)',
                        'Cache service (proxy cache) timed out serving downloads',
                        'Patch content download failed (cache/download agent)')
    ...
    foreach ($lf in $logFiles) {
        ...
        $isFcsaFile   = ($lf.File.Name -match '(?i)^FileCacheServiceAgent')   # authoritative PATCH cache metrics (SendManager etc have their own small caches)
        # every PME component log EXCEPT the two precisely-parsed ones only
        # matters for error/warn/config lines - one cheap scan per line
        $fastOnly     = ($lf.Label -like 'PME-*' -and -not $isTpp -and -not $isActionLog)
        ...
        foreach ($line in $lines) {
            # fast path: skip chatter lines before any regex work (Win11 boxes
            # parse at ~8-10s/MB, so this is where the runtime goes)
            if ($fastOnly -and
                $line.IndexOf('error', [System.StringComparison]::OrdinalIgnoreCase) -lt 0 -and
                $line.IndexOf('WARN') -lt 0 -and
                $line.IndexOf(' OFF ') -lt 0 -and
                $line.IndexOf('CurrentCache[') -lt 0) { continue }
            ...
            # ---- cache usage / config tracking (newest wins; FCSA only) ------
            if ($isFcsaFile -and $line.IndexOf('CurrentCache[') -ge 0) {
            ...
            if ($isFcsaFile -and $line.IndexOf(' OFF ') -ge 0) {
            ...
            if ($isActionLog) {
                ...
                if ($line -match 'processing action finished\. Id:\s*([0-9a-fA-F-]{36}),\s*Error:\s*(.*?),\s*RebootRequired:\s*(True|False)') {
                    ...
                    elseif ($aErr -match '(?i)reboot is required') {
                        # not a failure - the install completed pending restart
                        Add-Row -Date $effTs -Product $aTitle -Source 'PME action' -Result 'SUCCESS-REBOOT' `
                                -Reason 'Install completed - reboot required to finalise' -Class (Get-PatchClass -Title $aTitle)
                    }
                    elseif ($aErr -match '(?i)not found in WUA catalog|pre-install search failed') {
                        Add-Row -Date $effTs -Product $aTitle -Source 'PME action' -Result 'FAILED' `
                                -Reason 'Approved patch was not present in the WUA catalog at install time (approval/metadata sync issue) - re-sync or re-approve in N-central' `
                                -Class (Get-PatchClass -Title $aTitle)
                    }
            ...
            # ---- 3PP engine error counting (in-window, patch/cache/core/action)
            if ($inWindow -and ($isPatchFile -or $isCacheFile -or $isPmeCore -or $isActionLog) -and $line -match '\bERROR\b') {
                $lbl = $null
                ...
                elseif ($line -match '(?i)is not located in metadata')                      { $lbl = 'Approved patch missing from device metadata (approval sync issue)' }
                elseif ($line -match '(?i)not found in WUA catalog|pre-install search failed') { $lbl = 'Approved patch not present in WUA catalog at install time' }
                ...
                if ($lbl) {
                    ...
                    if ($dlLabels -contains $lbl -and ($null -eq $newestDlErr -or $effTs -gt $newestDlErr)) { $newestDlErr = $effTs }
            ...
            if ((-not $isActionLog) -and ($code -ne '' -or $hasVerdict)) {   # ActionManager has its own precise parser
                $prod = $null
    ...
    function Get-NormProduct {
        # normalise a product name for cross-source matching: drop arch/bitness
        # tokens so 'Firefox (x64) 152.0.6' and 'Mozilla Firefox 152.0.6' meet
        param([string]$p)
        $n = ($p -replace '(?i)\((x64|x86|64-bit|32-bit|arm64)\)', '' -replace '(?i)\b(x64|x86|64-bit|32-bit|arm64)\b', '')
        $n = (($n -replace '[^a-zA-Z0-9]', '').ToLower())
    ...
    foreach ($r in ($script:Rows | Sort-Object { $srcPref[$_.Source] }, Date)) {
        $normProd = Get-NormProduct $r.Product
    ...
    # DOWNLOAD-class error means the download pipeline has come back (e.g.
    # after a remediation) - historical errors must not trigger a re-clear
    # that would knock a recovering engine over again. Recovery is only about
    # the download pipeline: a device stuck in a metadata/WUA-catalog loop is
    # NOT recovered just because catalogs download.
    $seriousNonDl = 0
    foreach ($k in $engineErrors.Keys) {
        if ($k -match 'metadata|WUA catalog') { $seriousNonDl += $engineErrors[$k].N }
    }
    $engineRecovering = $false
    if ($engineUnhealthy -and $lastCatalogOk -and $newestDlErr -and
        $seriousNonDl -lt 10 -and $lastCatalogOk -gt $newestDlErr) {
        $engineUnhealthy  = $false
    ...
    $healthLabel = if ($engineRecovering) {
                       "RECOVERING - download errors have stopped and a catalog download succeeded {0:yyyy-MM-dd HH:mm} (after the last download error {1:yyyy-MM-dd HH:mm}); waiting for detection/installs to resume - no action taken" -f $lastCatalogOk, $newestDlErr
                   }
    ...
    if ($cacheConfig -and $cacheConfig.ProxyHost) {
        ...
        $proxyLbl = 'Cache service (proxy cache) timed out serving downloads'
        if ($engineErrors.ContainsKey($proxyLbl)) {
            $pe = $engineErrors[$proxyLbl]
            if (((Get-Date) - $pe.Last).TotalDays -le 7) {
                ...
                Write-Output ('   (proxy-cache timeouts occurred earlier in the window - {0}x, last {1:yyyy-MM-dd} - none recently)' -f $pe.N, $pe.Last)
            }
        }
    }
    if ($engineErrTotal -gt 0) {
    ...
    elseif ($prevRem -and ((Get-Date) - $prevRem.Time).TotalHours -lt $RemediationCooldownHours) {
        ...
        $stopped = @()
        $depStopped = @()   # dependents silently stopped by -Force - MUST be restarted too
        foreach ($sn in $stopOrder) {
            try {
                $svc = Get-Service -Name $sn -ErrorAction SilentlyContinue
                if ($svc -and $svc.Status -eq 'Running') {
                    $deps = @()
                    try { $deps = @($svc.DependentServices | Where-Object { $_.Status -eq 'Running' } | ForEach-Object { $_.Name }) } catch { }
                    Stop-Service -Name $sn -Force -ErrorAction Stop
                    $stopped += $sn
                    Write-Log ("  STOPPED service '{0}'." -f $sn)
                    foreach ($d in $deps) {
                        if ($stopped -notcontains $d -and $depStopped -notcontains $d) {
                            $depStopped += $d
                            Write-Log ("  (dependent service '{0}' was stopped along with '{1}' - it WILL be restarted)" -f $d, $sn) 'WARN'
                        }
                    }
                }
            }
        ...
        foreach ($d in $depStopped) { if ($toStart -notcontains $d) { $toStart += $d } }   # dependents last (Start-Service auto-starts requirements)
        $startFailures = 0
        ...
        # SAFETY NET: every service that was Running at Step 1 must be Running
        # again now - nothing in the platform stack may be left down.
        foreach ($sn in @($script:SvcStates.Keys)) {
            if ($script:SvcStates[$sn] -ne 'Running') { continue }
            try {
                $svc = Get-Service -Name $sn -ErrorAction SilentlyContinue
                if ($svc -and $svc.Status -ne 'Running') {
                    Write-Log ("  Service '{0}' was Running before remediation but is now {1} - starting it." -f $sn, $svc.Status) 'WARN'
                    try { Start-Service -Name $sn -ErrorAction Stop; Write-Log ("  STARTED service '{0}'." -f $sn) }
                    catch { $startFailures++; Write-Log ("  Could not start '{0}': {1}" -f $sn, $_.Exception.Message) 'ERROR' }
                }
            } catch { }
        }
