...
.NOTES
    ...
    Version : v4.8 (symmetric ScanHealth escalation - non-zero
...
function Invoke-ComponentStoreRepair {
    ...
    if (-not ($checkFlaggedDamage -or $ForceFullDISM)) {
        ...
        if ($r.TimedOut) {
            ...
            $exitTxt = Format-ExitCode $r
            Add-Remediation -Step "DISM /ScanHealth" -Result "FAILED" -DurationSec $r.Duration `
                -ErrorMessage "$exitTxt; treating as 'damage suspected' - escalating to RestoreHealth (see C:\Windows\Logs\DISM\dism.log)"
        }
    }
