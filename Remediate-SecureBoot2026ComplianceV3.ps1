...
.NOTES
    ...
    Version   : V3
    File      : Remediate-SecureBoot2026ComplianceV3.ps1
...
# -----------------------------------------------------------------------------
Log "Starting Secure Boot 2026 remediation $ScriptVersion on $env:COMPUTERNAME (Apply=$Apply)"
...
if ($allCertsPresent -and ($state.CurrentAvailable -eq $CompletionValue -or $null -eq $state.CurrentAvailable)) {
    ...
    Write-Output ("SUMMARY|Ver=$ScriptVersion|Host={0}|Status=Compliant|Action=None|AvailableUpdates={1}" -f $env:COMPUTERNAME, $_auText)
    exit 0
}
...
if ($desiredMask -eq 0) {
    ...
    foreach ($n in $pendingForLater) { Log $n 'NOTE' }
    Write-Output ("SUMMARY|Ver=$ScriptVersion|Host={0}|Status=Waiting|Action=None|AvailableUpdates={1}" -f $env:COMPUTERNAME, $_auText)
    exit 0
}
...
if (-not $Apply) {
    ...
    Write-Output ("SUMMARY|Ver=$ScriptVersion|Host={0}|Status=Dryrun|DesiredMask=0x{1:X4}|Actions={2}" -f $env:COMPUTERNAME, $desiredMask, $plannedActions.Count)
    exit 20
}
...
if ($AutoReboot) {
    ...
    try {
        ...
        Write-Output ("SUMMARY|Ver=$ScriptVersion|Host={0}|Status=Applied+RebootScheduled|NewMask=0x{1:X4}|RebootIn={2}s" -f $env:COMPUTERNAME, $newValue, $RebootDelaySeconds)
        exit 11
    } catch {
...
Write-Output ("SUMMARY|Ver=$ScriptVersion|Host={0}|Status=Applied+RebootRequired|NewMask=0x{1:X4}" -f $env:COMPUTERNAME, $newValue)
exit 10
