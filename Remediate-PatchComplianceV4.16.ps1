...
.NOTES
    ...
    Version : v4.16 (Per-step "timeout NN min" log messages updated
...
function Invoke-CapturedCommandWithTimeout {
    ...
    try {
        ...
        # v4.16: when ExitCode comes back null/empty on a step that did NOT
        # time out, surface the captured output's tail so the operator can
        # tell whether the process actually succeeded (and PowerShell just
        # could not read the exit code) or genuinely errored. The wrapper
        # could not read ExitCode but the process's own stdout/stderr is
        # ground truth. Progress-bar lines are filtered, blanks are dropped.
