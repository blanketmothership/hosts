...
.NOTES
    ...
    v1.1.0 - Use HKLM\...\SecureBoot\Servicing UEFICA2023Status as the
             authoritative signal (per Microsoft guidance); WindowsUEFICA2023Capable
             is now reference-only and no longer gates. On Windows Server the cert
             is NOT auto-delivered by Windows Update/CFR, so a patched-but-not-opted-in
             server correctly shows "no status" without being flagged as a failure.
             Added UEFICA2023Error detection as a real failure condition. Demoted
             TPM to informational (not required for the cert update).
...
# ---------------------------------------------------------------------------
# 6. Servicing-stack readiness (authoritative status from \Servicing subkey)
# ---------------------------------------------------------------------------
#   Per Microsoft guidance, UEFICA2023Status is the authoritative signal.
#   WindowsUEFICA2023Capable is "for reference only - do not use when getting
#   status." It is still reported below as reference, but does NOT gate.
#
#   On Windows Server the cert is NOT delivered automatically by Windows
#   Update / CFR. A fully patched server that has not yet been opted in will
#   legitimately show no status here - that is the expected pre-opt-in state,
#   NOT a missing-patch failure. The true prereq is only that the servicing
#   plumbing exists (Secure-Boot-Update task, check 8). We surface the status
#   values as informational state, and flag a hard error only if the servicing
#   stack reports an actual deployment error (UEFICA2023Error present).
$sbKey     = 'HKLM:\SYSTEM\CurrentControlSet\Control\Secureboot'
$servKey   = 'HKLM:\SYSTEM\CurrentControlSet\Control\SecureBoot\Servicing'

$servProps = $null
try {
    $servProps = Get-ItemProperty -Path $servKey -ErrorAction Stop
} catch { }

$uefiStatus  = $servProps.UEFICA2023Status            # e.g. 'Updated'
$capable     = $servProps.WindowsUEFICA2023Capable    # reference only: 0/1/2
$uefiErrRaw  = $servProps.UEFICA2023Error             # presence indicates a deployment error

$statusText  = if ($uefiStatus) { $uefiStatus } else { 'Not started (no opt-in yet)' }
$capableText = if ($null -eq $capable) { 'absent' } else { "$capable" }
$errText     = if ($null -ne $uefiErrRaw) { ('0x{0:x}' -f [int64]$uefiErrRaw) } else { 'none' }

# Informational state line (never a prereq failure on its own).
Add-Check -Name 'Servicing status (info)' -Pass $true `
    -Detail ("UEFICA2023Status: {0} | Capable(ref-only): {1} | Error: {2}" `
             -f $statusText, $capableText, $errText)

# A populated UEFICA2023Error IS a real failure worth surfacing.
$errIsRealFailure = ($null -ne $uefiErrRaw -and [int64]$uefiErrRaw -ne 0)
if ($errIsRealFailure) {
    Add-Check -Name 'No servicing deployment error' -Pass $false `
        -Detail "UEFICA2023Error present: $errText (check Event ID 1801 / DB-DBX update events)" `
        -MissingMsg "Servicing reports a Secure Boot deployment error ($errText). Investigate before proceeding (Event ID 1801, vendor firmware)."
} else {
    Add-Check -Name 'No servicing deployment error' -Pass $true `
        -Detail 'No UEFICA2023Error recorded'
}
...
# ---------------------------------------------------------------------------
# 9. TPM present and ready (INFORMATIONAL ONLY - not required for cert update)
# ---------------------------------------------------------------------------
#   The Secure Boot DB/KEK certificate update does NOT depend on TPM. This is
#   reported for awareness only and never contributes to the MISSING list.
#   (Common case: Gen 2 VM with no vTPM attached -> expected, harmless here.)
$tpmDetail = 'Unknown'
try {
...
Add-Check -Name 'TPM state (info)' -Pass $true `
    -Detail "$tpmDetail (not required for cert update)"
