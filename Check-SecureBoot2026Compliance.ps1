...
.NOTES
    ...
    Exit codes:
        0  = Compliant         (all 2023 certs present, boot mgr updated)
        1  = Non-Compliant     (one or more required items missing)
        2  = Not Applicable    (legacy BIOS / Secure Boot disabled / VM w/o UEFI)
...
if ($result.FirmwareType -notmatch 'Uefi') {
    ...
    Add-Issue "System is not booting in UEFI mode (FirmwareType=$($result.FirmwareType))." `
              "Secure Boot requires UEFI. Convert the disk to GPT and enable UEFI if this device should be compliant."
}
...
try {
    ...
    Add-Issue "Platform does not support Secure Boot." `
              "Hardware or firmware does not expose Secure Boot variables. No action possible."
} catch {
    $result.SecureBootSupported = $true
    $result.SecureBootEnabled   = $false
    Add-Issue "Secure Boot is disabled in firmware: $($_.Exception.Message)" `
              "Enable Secure Boot in the UEFI/BIOS setup. The 2023 certificate updates only apply when Secure Boot is on."
}

# If Secure Boot isn't available, no point continuing with cert checks.
    ...
    try {
        if ($espLetter) {
            ...
            Add-Issue "Could not locate or mount the EFI System Partition." `
                      "Manually inspect bootmgfw.efi signer with: Get-AuthenticodeSignature after mounting the ESP (mountvol X: /S)."
        }
    } catch {
...
# -----------------------------------------------------------------------------
if ($result.ComplianceStatus -eq 'Unknown') {
    if ($result.KEK2023Present -and $result.DbWindowsCA2023 -and
        $result.DbThirdPartyCA2023 -and $result.BootMgrSignedBy2023) {
