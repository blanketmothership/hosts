...
.NOTES
    Get-NCentralThirdPartyPatchReport_V1.9.ps1
...
try {
    ...
    foreach ($lf in $logFiles) {
        ...
        $isPmeCore   = ($lf.Label -like 'PME-*' -and $lf.File.Name -match '(?i)^Core\.log')
    ...
    foreach ($f in @($kept | Where-Object { $_.Result -eq 'FAILED' })) {
