...
.NOTES
    ...
    Version : v4.3 (hard time budget + bounded long-running ops:
...
param(
    ...
    # Hard wall-clock budget for the entire script. N-central's "Run a
    # Script" feature kills any task that exceeds 60 minutes, so we
    # default to 50 minutes to leave 10-min headroom for reporting,
    # transcript flushing, and N-central buffer drain.
...
function Invoke-CapturedCommandWithTimeout {
    ...
    # Cap requested timeout at remaining script budget
...
function Invoke-DiskCleanupRemediation {
    ...
    # 1. DISM /StartComponentCleanup  (v4.3: bounded + budget-gated)
    #
    #    /StartComponentCleanup can run for an hour on a damaged store. We
    #    cap the wall-clock at 15 min, and also defer it entirely when:
    #      - the WU stack itself is degraded (the component store repair
    #        in Phase 2 is the real cure; cleanup is just disk hygiene)
    #      - or remaining script budget < 30 min (cleanup is not worth
    #        consuming the budget that RestoreHealth/SFC will need).
    $cleanupSkipReason = $null
    $wuS = $Script:Results["WUServices"]
    if ($wuS -and -not $wuS["OverallHealthy"]) {
        $cleanupSkipReason = "WU services degraded; deferring to component store repair phase"
    } elseif ((Get-RemainingBudgetSec) -lt 1800) {
    ...
    if ($cleanupSkipReason) {
        ...
        $r = Invoke-CapturedCommandWithTimeout -FilePath "dism.exe" `
                ...
                -TimeoutSec 900 -Label "DISM StartComponentCleanup"
        if ($r.TimedOut) {
            Add-Remediation -Step "DISM StartComponentCleanup" -Result "FAILED" -DurationSec $r.Duration `
                -ErrorMessage "TIMEOUT after $([int]$r.Duration)s; component store likely damaged - Phase 2 RestoreHealth will retry"
...
function Invoke-ComponentStoreRepair {
    ...
    if (-not (Test-RemainingBudget -NeededSec 60)) {
        ...
        $r = Invoke-CapturedCommandWithTimeout -FilePath "dism.exe" `
                ...
                -TimeoutSec 300 -Label "DISM /CheckHealth"
        if ($r.TimedOut) {
            $checkFlaggedDamage = $true
            Add-Remediation -Step "DISM /CheckHealth" -Result "FAILED" -DurationSec $r.Duration `
                -ErrorMessage "TIMEOUT after $([int]$r.Duration)s; treating as 'damage suspected' and proceeding to ScanHealth"
    ...
    if (-not ($checkFlaggedDamage -or $ForceFullDISM)) {
        ...
        $r = Invoke-CapturedCommandWithTimeout -FilePath "dism.exe" `
                ...
                -TimeoutSec 600 -Label "DISM /ScanHealth"
        if ($r.TimedOut) {
            $needRestore = $true
            Add-Remediation -Step "DISM /ScanHealth" -Result "FAILED" -DurationSec $r.Duration `
                -ErrorMessage "TIMEOUT after $([int]$r.Duration)s; assuming damage and proceeding to RestoreHealth"
        } elseif ($r.ExitCode -eq 0) {
            if ($r.Output -match 'No component store corruption detected') {
    ...
    if (-not $needRestore) {
        ...
        $r = Invoke-CapturedCommandWithTimeout -FilePath "dism.exe" `
                ...
                -TimeoutSec 1500 -Label "DISM /RestoreHealth"
        if ($r.TimedOut) {
            Add-Remediation -Step "DISM /RestoreHealth" -Result "FAILED" -DurationSec $r.Duration `
                -ErrorMessage "TIMEOUT after $([int]$r.Duration)s; component store damage too deep for online repair - rebuild candidate"
        } elseif ($r.ExitCode -eq 0) {
    ...
    if (-not $runSfc) {
        ...
        $r = Invoke-CapturedCommandWithTimeout -FilePath "$env:SystemRoot\System32\sfc.exe" `
                ...
                -TimeoutSec 900 -Label "sfc /scannow"
        if ($r.TimedOut) {
...
# ============================================================
Write-Log "############################################################"
Write-Log " Remediate-PatchCompliance - Starting  [v4.3]"
...
Write-Log " (configurable via -MaxRuntimeMinutes; default 50 leaves 10-min headroom under N-central's 60-min wall)"
Write-Log "############################################################"
