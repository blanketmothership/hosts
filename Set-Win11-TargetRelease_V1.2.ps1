# =============================================================================
# Set-Win11-TargetRelease_V1.2.ps1
...
#
# V1.2: removes DisableOSUpgrade when the pin is active, since it blocks ALL
#       WU-offered feature upgrades (including the pinned 24H2 -> 25H2 move).
#
# Logic:
...
#   4b. If the pin is active and DisableOSUpgrade exists -> REMOVE it.
#       If the pin is NOT active, leave it alone (it may be the device's only
#       feature-upgrade block) and log a warning instead.
...
try {
    ...
    if (Test-Path -Path $PolicyPath) {
        ...
        # ---- 4b. DisableOSUpgrade handling --------------------------------------
        # DisableOSUpgrade=1 suppresses ALL WU-offered feature upgrades, which
        # would stop the pinned 25H2 upgrade from ever being offered.
        if ($existingValues -contains 'DisableOSUpgrade') {
            if ($allSet) {
                $val = (Get-ItemProperty -Path $PolicyPath -Name 'DisableOSUpgrade').DisableOSUpgrade
                Remove-ItemProperty -Path $PolicyPath -Name 'DisableOSUpgrade' -Force
                Write-Log "DisableOSUpgrade (was $val) REMOVED - it blocks WU-offered feature upgrades and would prevent the $TargetReleaseInfo pin from working."
            }
            else {
                Write-Log "WARNING: DisableOSUpgrade present but LEFT IN PLACE (pin is not active; removing it would unblock all feature upgrades on this device)."
            }
        }
    }
    else {
    ...
    if ($RunUpdateScan) {
        ...
        try {
            ...
            elseif ($currentRelease -eq $TargetReleaseInfo) {
                ...
                Write-Log "FEATURE UPDATE STATUS: $TargetReleaseInfo NOT offered in this scan (device on $currentRelease; not yet released to this device or blocked by a safeguard hold). Note: a just-removed DisableOSUpgrade may need a follow-up scan to take effect."
            }
        }
        catch {
