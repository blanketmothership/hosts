...
.NOTES
    ...
    Version : v4.24 (Field fixes from the V4.23 live run: bookkeeping
...
function Test-PendingReboot {
    ...
    # Combined convenience flag (serious OR PFR). v4.22: informational only -
    # the Phase 2 gate keys off RebootPending (serious sources), never PFR.
...
function Invoke-WUDetection {
    ...
    if ($InstallApplicable -and $rebP -and $rebP["RebootPending"]) {
        ...
        try {
            ...
            $installResult = if ($useMU) { Install-WindowsUpdate -AcceptAll -IgnoreReboot -MicrosoftUpdate -Confirm:$false -ErrorAction Stop }
                             else        { Install-WindowsUpdate -AcceptAll -IgnoreReboot -Confirm:$false -ErrorAction Stop }
            $dur = ((Get-Date) - $start).TotalSeconds
